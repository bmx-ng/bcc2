...
Check(sizeToLong.Exists() And sizeToLong.kind = CONVERSION_NUMERIC_WIDENING, "Size_T retains the legacy native-count conversion to Long")
Local sizeToULong:TConversion = nativeSizeConversions.Classify(legacyMemoryModel.BuiltinType("Size_T"), legacyMemoryModel.BuiltinType("ULong"))
Check(sizeToULong.Exists() And sizeToULong.kind = CONVERSION_NUMERIC_WIDENING, "Size_T to unsigned ULong is lossless on 32- and 64-bit targets")
...
Local inheritedGenericImplementationSource:String = "SuperStrict~nInterface IIterator<T>~nEnd Interface~nInterface IIterable<T>~nMethod GetIterator:IIterator<T>()~nEnd Interface~nInterface ICollection<T> Extends IIterable<T>~nMethod CopyTo(array:T[], index:Int = 0)~nEnd Interface~nType TGenericCollection<T> Implements ICollection<T>~nMethod GetIterator:IIterator<T>()~nReturn Null~nEnd Method~nMethod CopyTo(array:T[], index:Int = 0)~nEnd Method~nEnd Type~nType TStringCollection<T> Extends TGenericCollection<T>~nEnd Type~nLocal values:TStringCollection<String> = New TStringCollection<String>"
Local inheritedGenericImplementationParse:TParseResult = TBlitzMaxParser.ParseText(inheritedGenericImplementationSource, "inherited-generic-implementation.bmx")
Local inheritedGenericImplementationModel:TSemanticModel = TBlitzMaxSemanticAnalyzer.Analyze(inheritedGenericImplementationParse.syntaxTree)
TExpressionBinder.Bind(inheritedGenericImplementationModel)
Local inheritedGenericImplementationType:TSymbol = inheritedGenericImplementationModel.globalScope.LookupLocal("TStringCollection")[0]
Check(Not inheritedGenericImplementationModel.IsAbstractType(inheritedGenericImplementationType) And inheritedGenericImplementationModel.AbstractObligations(inheritedGenericImplementationType).length = 0 And Not HasDiagnostic(inheritedGenericImplementationModel.diagnostics, "BMX3316"), "constructed generic base methods satisfy inherited generic Interface obligations")
