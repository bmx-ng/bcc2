...
Local genericStringOrderingSource:String = "SuperStrict~nType TFoo<T>~nField val:T~nMethod Operator <:Int(other:TFoo<T>)~nReturn val < other.val~nEnd Method~nMethod Operator >:Int(other:TFoo<T>)~nReturn val > other.val~nEnd Method~nMethod AtMost:Int(other:TFoo<T>)~nReturn val <= other.val~nEnd Method~nMethod AtLeast:Int(other:TFoo<T>)~nReturn val >= other.val~nEnd Method~nEnd Type~nLocal f1:TFoo<String> = New TFoo<String>()~nLocal f2:TFoo<String> = New TFoo<String>()"
Local genericStringOrderingCompilation:TCompilerResult = TBlitzMaxCompiler.Compile("generic-string-ordering.bmx", genericStringOrderingSource, Null, compilerOptions)
Local genericStringOrderingImplementation:String
If genericStringOrderingCompilation.genericPlan And genericStringOrderingCompilation.genericPlan.units.length Then genericStringOrderingImplementation = genericStringOrderingCompilation.genericPlan.units[0].implementation
Check(genericStringOrderingCompilation.Succeeded() And Occurrences(genericStringOrderingImplementation, "bbStringCompare(") = 4 And Not HasCompilerDiagnostic(genericStringOrderingCompilation, "BMXC3047"), "closed generic String ordering uses the managed runtime comparison helper: " + CompilationSummary(genericStringOrderingCompilation))
...
Local failedTypeSpecializationSource:String = "SuperStrict~nType TInvalid<T>~nMethod Remainder:T(left:T, right:T)~nReturn left Mod right~nEnd Method~nEnd Type~nLocal invalid:TInvalid<Float> = New TInvalid<Float>()"
Local failedTypeSpecializationCompilation:TCompilerResult = TBlitzMaxCompiler.Compile("generic-failed-type-specialization.bmx", failedTypeSpecializationSource, Null, compilerOptions)
Check(HasCompilerDiagnostic(failedTypeSpecializationCompilation, "BMXC3047") And Not HasCompilerDiagnostic(failedTypeSpecializationCompilation, "BMXC1015"), "a failed generic specialization does not cascade into an unimplemented New expression during ordinary IR lowering")
