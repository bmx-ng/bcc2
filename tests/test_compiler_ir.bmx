...
Function EmissionDiagnosticSummary:String(diagnostics:TCompilerDiagnostic[])
	Local summary:String
...
resolver.core = TSnapshotText.Create("sdk/blitz_classes.i", "Object^Null{~n-ToString:String()=~qbbObjectToString~q~n-Compare:Int(other:Object)=~qbbObjectCompare~q~n-SendMessage:Object(message:Object,source:Object)=~qbbObjectSendMessage~q~n-HashCode:UInt()=~qbbObjectHashCode~q~n-Equals:Int(other:Object)=~qbbObjectEquals~q~n}=~qbbObjectClass~q~nString^Object{~n@length%~n-ToString:String()=~qbbStringToString~q~n-Compare:Int(other:String)=~qbbStringCompare~q~n-Compare:Int(other:String,caseSensitive:Int)=~qbbStringCompareCase~q~n-HashCode:UInt()=~qbbStringHash~q~n-HashCode:UInt(caseSensitive:Int)=~qbbStringHashCase~q~n-Equals:Int(other:String)=~qbbStringEquals~q~n-Equals:Int(other:String,caseSensitive:Int)=~qbbStringEqualsCase~q~n-Find:Int(subString:String,startIndex:Int=0)=~qbbStringFind~q~n-Replace:String(subString:String,withString:String)=~qbbStringReplace~q~n+FromInt:String(value:Int)=~qbbStringFromInt~q~n}AF=~qbbStringClass~q~n___Array^Object{~n@length%~n}AF=~qbbArrayClass~q~nMilliSecs%()=~qbbMilliSecs~q")
resolver.AddInterface("brl.blitz", "sdk/brl.blitz.i", "superstrict")
Local automaticSourceMode:TCompilerResult = TBlitzMaxCompiler.Compile("automatic-source-mode.bmx", "Local value:Int = 1", resolver, TestOptions())
Check(automaticSourceMode.Succeeded() And automaticSourceMode.analysis.syntaxTree.root.sourceMode = SOURCE_MODE_SUPERSTRICT, "compiler defaults undeclared source mode to SuperStrict")
Local noAutomaticOptions:TCompilerOptions = TestOptions()
noAutomaticOptions.noAutoSuperStrict = True
Local disabledAutomaticMode:TCompilerResult = TBlitzMaxCompiler.Compile("no-automatic-source-mode.bmx", "Local value:Int = 1", resolver, noAutomaticOptions)
Check(disabledAutomaticMode.Succeeded() And disabledAutomaticMode.analysis.syntaxTree.root.sourceMode = SOURCE_MODE_STRICT, "compiler -nas uses Strict for an undeclared source mode")
Local explicitStrictSourceMode:TCompilerResult = TBlitzMaxCompiler.Compile("explicit-strict-source-mode.bmx", "Strict~nLocal value:Int = 1", resolver, TestOptions())
Check(explicitStrictSourceMode.Succeeded() And explicitStrictSourceMode.analysis.syntaxTree.root.sourceMode = SOURCE_MODE_STRICT, "compiler preserves an explicit Strict declaration")
...
Check(Contains(gdbC, "SourceMapped(BBINT value) {") And Contains(gdbC, "BBINT nextValue") And Contains(gdbC, "value + 1") And Not Contains(gdbC, "bmx_v0_nextValue"), "gdb C publishes source-facing parameter and local names to native debugger variable views")
...
Local picoTargetSymbols:String[] = CompilerDefaultConditionalSymbols("pico", "arm", "release", "", False)
Check(HasConditionalSymbol(picoTargetSymbols, "pico") And HasConditionalSymbol(picoTargetSymbols, "embedded") And HasConditionalSymbol(picoTargetSymbols, "arm") And HasConditionalSymbol(picoTargetSymbols, "picoarm") And HasConditionalSymbol(picoTargetSymbols, "ptr32") And HasConditionalSymbol(picoTargetSymbols, "longint4") And HasConditionalSymbol(picoTargetSymbols, "littleendian") And Not HasConditionalSymbol(picoTargetSymbols, "linux") And Not HasConditionalSymbol(picoTargetSymbols, "threaded"), "Pico/arm exposes the compact embedded ABI and its 32-bit little-endian platform aliases without desktop or threading aliases")
Local esp32TargetSymbols:String[] = CompilerDefaultConditionalSymbols("esp32", "xtensa", "release", "", False)
Check(HasConditionalSymbol(esp32TargetSymbols, "esp32") And HasConditionalSymbol(esp32TargetSymbols, "embedded") And HasConditionalSymbol(esp32TargetSymbols, "xtensa") And HasConditionalSymbol(esp32TargetSymbols, "esp32xtensa") And HasConditionalSymbol(esp32TargetSymbols, "ptr32") And HasConditionalSymbol(esp32TargetSymbols, "longint4") And HasConditionalSymbol(esp32TargetSymbols, "littleendian") And Not HasConditionalSymbol(esp32TargetSymbols, "pico") And Not HasConditionalSymbol(esp32TargetSymbols, "threaded"), "ESP32/Xtensa exposes the compact embedded ABI and its 32-bit little-endian target aliases without Pico or threading aliases")
Local esp32RiscvTargetSymbols:String[] = CompilerDefaultConditionalSymbols("esp32", "riscv32", "release", "", False)
Check(HasConditionalSymbol(esp32RiscvTargetSymbols, "esp32") And HasConditionalSymbol(esp32RiscvTargetSymbols, "embedded") And HasConditionalSymbol(esp32RiscvTargetSymbols, "riscv32") And HasConditionalSymbol(esp32RiscvTargetSymbols, "esp32riscv32") And HasConditionalSymbol(esp32RiscvTargetSymbols, "ptr32") And HasConditionalSymbol(esp32RiscvTargetSymbols, "longint4") And HasConditionalSymbol(esp32RiscvTargetSymbols, "littleendian") And Not HasConditionalSymbol(esp32RiscvTargetSymbols, "xtensa") And Not HasConditionalSymbol(esp32RiscvTargetSymbols, "threaded"), "ESP32/RISC-V exposes the compact embedded ABI and its 32-bit little-endian target aliases without Xtensa or threading aliases")
Local supportedTargets:String[] = ["macos/x86", "macos/x64", "macos/ppc", "macos/arm64", "win32/x86", "win32/x64", "win32/armv7", "win32/arm64", "linux/x86", "linux/x64", "linux/arm", "linux/arm64", "linux/riscv32", "linux/riscv64", "haiku/x86", "haiku/x64", "haiku/arm64", "pico/arm", "esp32/xtensa", "esp32/riscv32"]
For Local supportedTarget:String = EachIn supportedTargets
	Local supportedParts:String[] = supportedTarget.Split("/")
	Check(CompilerTargetSupported(supportedParts[0], supportedParts[1]), "compiler accepts supported target " + supportedTarget)
Next
Local rejectedTargets:String[] = ["macos/riscv64", "win32/arm", "linux/ppc", "windows/x64", "linux/unknown", "pico/arm64", "pico/riscv32", "esp32/arm", "esp32/riscv64"]
For Local rejectedTarget:String = EachIn rejectedTargets
	Local rejectedParts:String[] = rejectedTarget.Split("/")
	Check(Not CompilerTargetSupported(rejectedParts[0], rejectedParts[1]), "compiler rejects unsupported target " + rejectedTarget)
Next

Local noRuntimeResolver:TCompilerTestResolver = New TCompilerTestResolver
noRuntimeResolver.core = resolver.core
noRuntimeResolver.AddInterface("brl.blitz", "sdk/brl.blitz.i", "superstrict~nMemAlloc@*(size%z,collectable%=0%)=~qbrl_blitz_MemAlloc__Bsize_t__Bint~q~nMemFree(mem@*,collectable%=0%)=~qbrl_blitz_MemFree__PBbyte__Bint~q~nMemExtend@*(mem@*,size%z,new_size%z,collectable%=0%)=~qbrl_blitz_MemExtend__PBbyte__Bsize_t__Bsize_t__Bint~q~nMemClear(mem@*,size%z)=~qbbMemClear~q~nMemCopy(dst@*,src@*,size%z)=~qbbMemCopy~q~nMemMove(dst@*,src@*,size%z)=~qbbMemMove~q")
Local noRuntimeOptions:TCompilerOptions = TestOptions()
noRuntimeOptions.targetPlatform = "pico"
noRuntimeOptions.targetArchitecture = "arm"
noRuntimeOptions.threaded = False
noRuntimeOptions.implicitRuntime = False
noRuntimeOptions.RefreshConditionalSymbols()
noRuntimeResolver.AddInterface("pico.contracts", "sdk/pico.contracts.i", "superstrict~nIReading^Object{~n-Read%(value%)A=~qpico_contracts_IReading_Read__Bint~q~n}AI=~qpico_contracts_IReading~q~nILabelled^IReading{~n-Label$()A=~qpico_contracts_ILabelled_Label~q~n}AI=~qpico_contracts_ILabelled~q")
noRuntimeResolver.AddInterface("pico.incompletecontracts", "sdk/pico.incompletecontracts.i", "superstrict~nIIncomplete^Object{~n-Read%()A~n}AI")
noRuntimeResolver.AddInterface("pico.types", "sdk/pico.types.i", "superstrict~nIStable^Object{~n-Stable%()A=~qpico_types_IStable_Stable~q~n}AI=~qpico_types_IStable~q~nTImportedReading^Object@IStable{~n.value%&~n.label$&~n-New(initial%)=~qpico_types_TImportedReading_New__Bint~q~n-Read%(value%)=~qpico_types_TImportedReading_Read__Bint~q~n-Stable%()=~qpico_types_TImportedReading_Stable~q~n-Delete()=~qpico_types_TImportedReading_Delete~q~n}=~qpico_types_TImportedReading~q~nCreateImportedReading:TImportedReading()=~qpico_types_CreateImportedReading~q")
noRuntimeResolver.AddInterface("pico.typefunctions", "sdk/pico.typefunctions.i", "superstrict~nTFactoryBase^Object{~n+Find:TFactoryBase(name$)=~qpico_typefunctions_TFactoryBase_Find__Bstring~q~n-Name$()A=~qpico_typefunctions_TFactoryBase_Name~q~n}A=~qpico_typefunctions_TFactoryBase~q")
Local noRuntimeScalar:TCompilerResult = TBlitzMaxCompiler.Compile("pico-scalar.bmx", "SuperStrict~nLocal value:Int = 20~nvalue :* 2", noRuntimeResolver, noRuntimeOptions)
Local noRuntimeScalarDiagnostics:TCompilerDiagnostic[]
Local noRuntimeScalarC:String = TBlitzMaxCompiler.EmitC(noRuntimeScalar, noRuntimeScalarDiagnostics)
Check(noRuntimeScalar.Succeeded() And noRuntimeScalarDiagnostics.length = 0 And Contains(noRuntimeScalarC, "int main(void)") And Contains(noRuntimeScalarC, "bmx_embedded_modules_init();") And Not Contains(noRuntimeScalarC, "bmx_embedded_root_frame_enter(&bmx_embedded_root_frame") And Contains(noRuntimeScalarC, "value *= 2;"), "runtime-free Pico scalar compilation retains the semantic-model to typed-IR to standalone-C pipeline and elides a root frame when the typed body has no GC safepoint")
Local noRuntimeDebugOptions:TCompilerOptions = TestOptions()
noRuntimeDebugOptions.targetPlatform = "pico"
noRuntimeDebugOptions.targetArchitecture = "arm"
noRuntimeDebugOptions.threaded = False
noRuntimeDebugOptions.implicitRuntime = False
noRuntimeDebugOptions.buildMode = "debug"
noRuntimeDebugOptions.debugInstrumentation = False
noRuntimeDebugOptions.RefreshConditionalSymbols()
Local embeddedAssertSource:String = "SuperStrict~nGlobal checks:Int~nFunction Condition:Int(value:Int)~nchecks:+1~nReturn value~nEnd Function~nFunction Message:String()~nchecks:+100~nReturn ~qfailure~q~nEnd Function~nAssert Condition(False), Message()"
Local embeddedDebugAssert:TCompilerResult = TBlitzMaxCompiler.Compile("embedded-debug-assert.bmx", embeddedAssertSource, noRuntimeResolver, noRuntimeDebugOptions)
Local embeddedDebugAssertDiagnostics:TCompilerDiagnostic[]
Local embeddedDebugAssertC:String = TBlitzMaxCompiler.EmitC(embeddedDebugAssert, embeddedDebugAssertDiagnostics)
Check(embeddedDebugAssert.Succeeded() And embeddedDebugAssertDiagnostics.length = 0 And Contains(TCompilerIrDumper.Dump(embeddedDebugAssert.ir), "assert @embedded-debug-assert.bmx:"), "embedded debug builds retain Assert independently of unavailable desktop debugger callbacks")
Check(Contains(embeddedDebugAssertC, "extern void brl_blitz_RuntimeError(const BMXEmbeddedString *message);") And Contains(embeddedDebugAssertC, "if (!(bmx_fn0_Condition(0))) {") And Contains(embeddedDebugAssertC, "brl_blitz_RuntimeError(bmx_fn1_Message());"), "embedded debug Assert uses the catchable BRL RuntimeError ABI with lazy message evaluation: " + EmissionDiagnosticSummary(embeddedDebugAssertDiagnostics))
Local embeddedReleaseAssert:TCompilerResult = TBlitzMaxCompiler.Compile("embedded-release-assert.bmx", embeddedAssertSource, noRuntimeResolver, noRuntimeOptions)
Local embeddedReleaseAssertDiagnostics:TCompilerDiagnostic[]
Local embeddedReleaseAssertC:String = TBlitzMaxCompiler.EmitC(embeddedReleaseAssert, embeddedReleaseAssertDiagnostics)
Check(embeddedReleaseAssert.Succeeded() And embeddedReleaseAssertDiagnostics.length = 0 And Not Contains(TCompilerIrDumper.Dump(embeddedReleaseAssert.ir), "assert @embedded-release-assert.bmx:") And Not Contains(embeddedReleaseAssertC, "bmx_fn0_Condition(0)") And Not Contains(embeddedReleaseAssertC, "bmx_fn1_Message()"), "embedded release Assert elides both condition and lazy message evaluation")
Local picoIncbin:TCompilerResult = TBlitzMaxCompiler.Compile("pico-incbin.bmx", "SuperStrict~nIncbin ~qasset.dat~q~nLocal data:Byte Ptr=IncbinPtr(~qasset.dat~q)~nLocal size:Int=IncbinLen(~qasset.dat~q)", noRuntimeResolver, noRuntimeOptions)
Local picoIncbinDiagnostics:TCompilerDiagnostic[]
Local picoIncbinC:String = TBlitzMaxCompiler.EmitC(picoIncbin, picoIncbinDiagnostics)
Check(picoIncbin.Succeeded() And picoIncbinDiagnostics.length = 0 And Contains(picoIncbinC, "extern const unsigned char *_ib_bb_main_1_data;") And Contains(picoIncbinC, "extern const unsigned int _ib_bb_main_1_size;") And Contains(picoIncbinC, "bbIncbinAdd(&bmx_embedded_string_str0, &_ib_bb_main_1_data, (int32_t)_ib_bb_main_1_size);") And AppearsBefore(picoIncbinC, "bmx_embedded_modules_init();", "bbIncbinAdd("), "Pico standalone C declares packaged Incbin symbols and registers flash resources after imported modules initialize")
Local picoMilliSecs:TCompilerResult = TBlitzMaxCompiler.Compile("pico-millisecs.bmx", "SuperStrict~nLocal ticks:Int=MilliSecs()", noRuntimeResolver, noRuntimeOptions)
Local picoMilliSecsDiagnostics:TCompilerDiagnostic[]
Local picoMilliSecsC:String = TBlitzMaxCompiler.EmitC(picoMilliSecs, picoMilliSecsDiagnostics)
Check(picoMilliSecs.Succeeded() And picoMilliSecsDiagnostics.length = 0 And Contains(picoMilliSecsC, "bmx_embedded_millisecs(") And Not Contains(picoMilliSecsC, "bbMilliSecs("), "Pico standard MilliSecs lowers directly to the embedded monotonic clock")
Local picoStringEachIn:TCompilerResult = TBlitzMaxCompiler.Compile("pico-string-eachin.bmx", "SuperStrict~nLocal identifier:String=~qA1~q~nLocal total:Int~nFor Local character:Int=EachIn identifier~ntotal:+character~nNext", noRuntimeResolver, noRuntimeOptions)
Local picoStringEachInDiagnostics:TCompilerDiagnostic[]
Local picoStringEachInC:String = TBlitzMaxCompiler.EmitC(picoStringEachIn, picoStringEachInDiagnostics)
Check(picoStringEachIn.Succeeded() And picoStringEachInDiagnostics.length = 0 And Contains(picoStringEachInC, "const BMXEmbeddedString * bmx_tmp_") And Contains(picoStringEachInC, "uint32_t bmx_tmp_") And Contains(picoStringEachInC, "int32_t bmx_tmp_") And Not Contains(picoStringEachInC, "BBSTRING") And Not Contains(picoStringEachInC, "BBUINT") And Not Contains(picoStringEachInC, "BBINT"), "Pico String EachIn uses only compact embedded String, index and code-unit ABI names")
Local picoFixedObjectEachInSource:String = "SuperStrict~nType TEmbeddedEachItem~nEnd Type~nGlobal items:TEmbeddedEachItem[2]~nLocal count:Int~nFor Local item:TEmbeddedEachItem=EachIn items~nIf item Then count:+1~nNext"
Local picoFixedObjectEachIn:TCompilerResult = TBlitzMaxCompiler.Compile("pico-fixed-object-eachin.bmx", picoFixedObjectEachInSource, noRuntimeResolver, noRuntimeOptions)
Local picoFixedObjectEachInDiagnostics:TCompilerDiagnostic[]
Local picoFixedObjectEachInC:String = TBlitzMaxCompiler.EmitC(picoFixedObjectEachIn, picoFixedObjectEachInDiagnostics)
Check(picoFixedObjectEachIn.Succeeded() And picoFixedObjectEachInDiagnostics.length = 0 And Contains(picoFixedObjectEachInC, "struct bmx_cls0_TEmbeddedEachItem_obj * bmx_tmp_") And Contains(picoFixedObjectEachInC, "(void *)&bmx_embedded_null_object") And Not Contains(picoFixedObjectEachInC, "BBOBJECT"), "embedded fixed Object Array EachIn uses only precise embedded object pointer ABI names")
Local picoObjectStringEachInSource:String = "SuperStrict~nLocal values:Object[]=[~qtext~q]~nLocal found:String~nFor Local value:String=EachIn values~nfound=value~nNext"
Local picoObjectStringEachIn:TCompilerResult = TBlitzMaxCompiler.Compile("pico-object-string-eachin.bmx", picoObjectStringEachInSource, noRuntimeResolver, noRuntimeOptions)
Local picoObjectStringEachInDiagnostics:TCompilerDiagnostic[]
Local picoObjectStringEachInC:String = TBlitzMaxCompiler.EmitC(picoObjectStringEachIn, picoObjectStringEachInDiagnostics)
Check(picoObjectStringEachIn.Succeeded() And picoObjectStringEachInDiagnostics.length = 0 And Contains(picoObjectStringEachInC, "bmx_embedded_object_is_string((BMXEmbeddedObject *)") And Contains(picoObjectStringEachInC, "bmx_embedded_object_to_string((void *)") And Not Contains(picoObjectStringEachInC, "BBOBJECT"), "embedded Object Array to String EachIn filters and converts through the precise embedded ABI")
Local picoGenericScalarSource:String = "SuperStrict~nFunction Identity<T>:T(value:T)~nReturn value~nEnd Function~nGlobal answer:Int = Identity<Int>(42)"
Local picoGenericScalar:TCompilerResult = TBlitzMaxCompiler.Compile("pico-generic-scalar.bmx", picoGenericScalarSource, noRuntimeResolver, noRuntimeOptions)
Local picoGenericBuildDiagnostics:TCompilerDiagnostic[]
Local picoGenericBuild:TCompilerBuildOutputPlan = TBlitzMaxCompiler.PlanBuildOutputs(picoGenericScalar, "application.c", "", "", picoGenericBuildDiagnostics)
Local picoGenericUnit:TCompilerBuildOutputFile
Local picoGenericApplication:TCompilerBuildOutputFile
For Local file:TCompilerBuildOutputFile = EachIn picoGenericBuild.files
	If file.role = "generic-specialization-c" Then picoGenericUnit = file
	If file.role = "application-c" Then picoGenericApplication = file
Next
Check(picoGenericScalar.Succeeded() And picoGenericBuildDiagnostics.length = 0 And picoGenericBuild.linkInputs.length = 1 And picoGenericUnit And picoGenericUnit.content.Contains("#include <stdint.h>") And picoGenericUnit.content.Contains("int32_t") And Not picoGenericUnit.content.Contains("brl.mod/blitz.mod/blitz.h"), "Pico build bundles retain separately emitted scalar generic units without importing the desktop runtime ABI")
Check(picoGenericApplication And picoGenericApplication.content.Contains("/* Generated by bcc 1.00 scalar C99 backend. */"), "Pico build-output planning selects the standalone embedded C backend")
Local picoGenericArrayConcatSource:String = "SuperStrict~nType TJoiner<T>~nMethod Join:T[](left:T[],right:T[])~nReturn left+right~nEnd Method~nEnd Type~nLocal joiner:TJoiner<String>=New TJoiner<String>~nLocal joined:String[]=joiner.Join([~qa~q],[~qb~q])"
Local picoGenericArrayConcat:TCompilerResult = TBlitzMaxCompiler.Compile("pico-generic-array-concat.bmx", picoGenericArrayConcatSource, noRuntimeResolver, noRuntimeOptions)
Local picoGenericArrayConcatDiagnostics:TCompilerDiagnostic[]
Local picoGenericArrayConcatBuild:TCompilerBuildOutputPlan = TBlitzMaxCompiler.PlanBuildOutputs(picoGenericArrayConcat, "application.c", "", "", picoGenericArrayConcatDiagnostics)
Local picoGenericArrayConcatUnits:String
For Local file:TCompilerBuildOutputFile = EachIn picoGenericArrayConcatBuild.files
	If file.role = "generic-specialization-c" Then picoGenericArrayConcatUnits :+ file.content
Next
Check(picoGenericArrayConcat.Succeeded() And picoGenericArrayConcatDiagnostics.length = 0 And picoGenericArrayConcatUnits.Contains("BMXEmbeddedArray *bmx_array_concat_") And picoGenericArrayConcatUnits.Contains("BMX_EMBEDDED_ROOT_ARRAY") And picoGenericArrayConcatUnits.Contains("bmx_embedded_array_concat(bmx_array_concat_"), "Pico generic Array concatenation roots its sequenced left operand across evaluation and allocation: " + CompilerDiagnosticSummary(picoGenericArrayConcat) + "; emission: " + EmissionDiagnosticSummary(picoGenericArrayConcatDiagnostics))
Local picoGenericStructSource:String = "SuperStrict~nStruct SPair<T>~nField first:T~nField second:T~nMethod New(first:T,second:T)~nSelf.first=first~nSelf.second=second~nEnd Method~nMethod First:T()~nReturn first~nEnd Method~nMethod AppendFirst:T(suffix:T)~nLocal retained:T=first~nReturn retained+suffix~nEnd Method~nEnd Struct~nLocal pair:SPair<String>=New SPair<String>(~qfirst~q,~qsecond~q)~nLocal pairs:SPair<String>[]=New SPair<String>[2]~npairs[1]=pair~nLocal value:String=pairs[1].AppendFirst(~q suffix~q)"
Local picoGenericStruct:TCompilerResult = TBlitzMaxCompiler.Compile("pico-generic-struct.bmx", picoGenericStructSource, noRuntimeResolver, noRuntimeOptions)
Local picoGenericStructDiagnostics:TCompilerDiagnostic[]
Local picoGenericStructBuild:TCompilerBuildOutputPlan = TBlitzMaxCompiler.PlanBuildOutputs(picoGenericStruct, "application.c", "", "", picoGenericStructDiagnostics)
Local picoGenericStructUnit:TCompilerBuildOutputFile
Local picoGenericStructApplication:TCompilerBuildOutputFile
For Local file:TCompilerBuildOutputFile = EachIn picoGenericStructBuild.files
	If file.role = "generic-specialization-c" Then picoGenericStructUnit = file
	If file.role = "application-c" Then picoGenericStructApplication = file
Next
Check(picoGenericStruct.Succeeded(), "Pico accepts the allocation-free managed generic Struct source")
Check(picoGenericStructDiagnostics.length = 0, "Pico plans the managed generic Struct without build-output diagnostics: " + EmissionDiagnosticSummary(picoGenericStructDiagnostics))
Check(picoGenericStructBuild.linkInputs.length = 1 And picoGenericStructUnit, "Pico plans one separately emitted managed generic Struct specialization")
Check(picoGenericStructUnit.content.Contains("const BMXEmbeddedString *") And picoGenericStructUnit.content.Contains("&bmx_embedded_empty_string") And picoGenericStructUnit.content.Contains("bbStructElementInit_"), "Pico generic Struct units use the embedded managed-value ABI and publish element initialization")
Check(Not picoGenericStructUnit.content.Contains("BBArray") And Not picoGenericStructUnit.content.Contains("bbObjectRegisterStruct"), "Pico generic Struct units omit desktop Array and reflection registration dependencies")
Check(picoGenericStructUnit.content.Contains("const BMXEmbeddedValueDescriptor bmx_gen_") And picoGenericStructUnit.content.Contains("_pico_value_descriptor =") And picoGenericStructUnit.content.Contains("BMX_EMBEDDED_VALUE_STRING") And picoGenericStructUnit.content.Contains("BMX_EMBEDDED_ROOT_STRUCT") And picoGenericStructUnit.content.Contains("BMX_EMBEDDED_ROOT_STRING") And picoGenericStructUnit.content.Contains("bmx_embedded_root_frame_enter(&bmx_embedded_generic_root_frame") And picoGenericStructUnit.content.Contains("bmx_embedded_root_frame_leave(&bmx_embedded_generic_root_frame)") And picoGenericStructUnit.content.Contains("bmx_embedded_string_concat("), "Pico generic Struct units publish precise descriptors and root Self, managed parameters, and managed locals around allocating String operations")
Check(picoGenericStructApplication And picoGenericStructApplication.content.Contains("BMX_EMBEDDED_VALUE_STRING") And picoGenericStructApplication.content.Contains("BMX_EMBEDDED_ROOT_STRUCT") And picoGenericStructApplication.content.Contains("bmx_embedded_array_new_1d") And picoGenericStructApplication.content.Contains("bbStructElementInit_"), "Pico consumers emit precise descriptors and root slots for managed generic Struct values and Arrays")
Local picoNestedGenericStructSource:String = "SuperStrict~nStruct SCell<T>~nField value:T~nEnd Struct~nType TCellHolder<T>~nField retained:SCell<T>~nField values:SCell<T>[]~nMethod Expand:SCell<T>[](value:T)~nLocal source:SCell<T>[]=New SCell<T>[2]~nsource[1].value=value~nLocal localCell:SCell<T>=source[1]~nretained=localCell~nvalues=source[-1..3]~nReturn values~nEnd Method~nEnd Type~nGlobal holder:TCellHolder<String>=New TCellHolder<String>~nGlobal cells:SCell<String>[]=holder.Expand(~qkept~q)"
Local picoNestedGenericStruct:TCompilerResult = TBlitzMaxCompiler.Compile("pico-nested-generic-struct.bmx", picoNestedGenericStructSource, noRuntimeResolver, noRuntimeOptions)
Local picoNestedGenericStructDiagnostics:TCompilerDiagnostic[]
Local picoNestedGenericStructBuild:TCompilerBuildOutputPlan = TBlitzMaxCompiler.PlanBuildOutputs(picoNestedGenericStruct, "application.c", "", "", picoNestedGenericStructDiagnostics)
Local picoNestedGenericStructUnits:String
For Local file:TCompilerBuildOutputFile = EachIn picoNestedGenericStructBuild.files
	If file.role = "generic-specialization-c" Then picoNestedGenericStructUnits :+ file.content
Next
Check(picoNestedGenericStruct.Succeeded() And picoNestedGenericStructDiagnostics.length = 0 And picoNestedGenericStructBuild.linkInputs.length = 2, "Pico plans generic Struct Arrays nested in a generic Type as canonical specialization units")
Check(picoNestedGenericStructUnits.Contains("extern const BMXEmbeddedValueDescriptor bmx_gen_") And picoNestedGenericStructUnits.Contains("_pico_value_descriptor;") And picoNestedGenericStructUnits.Contains("bbStructElementInit_") And picoNestedGenericStructUnits.Contains("bmx_embedded_array_new_1d") And picoNestedGenericStructUnits.Contains("bmx_embedded_array_slice"), "Pico generic specialization units share the published Struct descriptor and initializer across allocation and slicing")
Check(picoNestedGenericStructUnits.Contains("BMX_EMBEDDED_VALUE_STRUCT") And picoNestedGenericStructUnits.Contains(".value_field_count = 1") And picoNestedGenericStructUnits.Contains("BMX_EMBEDDED_ROOT_STRUCT"), "Pico generic Type descriptors and root frames trace managed references nested inside generic Struct values")
Local picoAllocatingGenericStruct:TCompilerResult = TBlitzMaxCompiler.Compile("pico-allocating-generic-struct.bmx", "SuperStrict~nStruct SLabel<T>~nField value:T~nMethod Label:String()~nReturn ~qallocated~q~nEnd Method~nEnd Struct~nLocal label:SLabel<Int>~nLocal value:String=label.Label()", noRuntimeResolver, noRuntimeOptions)
Check(Not picoAllocatingGenericStruct.Succeeded() And HasCompilerDiagnostic(picoAllocatingGenericStruct, "BMXC3092"), "Pico diagnoses generic Struct bodies that require specialization-owned managed literal declarations before C emission")
Local picoGenericType:TCompilerResult = TBlitzMaxCompiler.Compile("pico-generic-type.bmx", "SuperStrict~nType TBox<T>~nField value:T~nMethod New(initial:T)~nvalue=initial~nEnd Method~nMethod Get:T()~nReturn value~nEnd Method~nEnd Type~nGlobal box:TBox<Int> = New TBox<Int>(42)~nGlobal boxedValue:Int=box.Get()", noRuntimeResolver, noRuntimeOptions)
Local picoGenericTypeDiagnostics:TCompilerDiagnostic[]
Local picoGenericTypeBuild:TCompilerBuildOutputPlan = TBlitzMaxCompiler.PlanBuildOutputs(picoGenericType, "application.c", "", "", picoGenericTypeDiagnostics)
Local picoGenericTypeUnit:TCompilerBuildOutputFile
For Local file:TCompilerBuildOutputFile = EachIn picoGenericTypeBuild.files
	If file.role = "generic-specialization-c" Then picoGenericTypeUnit = file
Next
Check(picoGenericType.Succeeded() And picoGenericTypeDiagnostics.length = 0 And picoGenericTypeUnit, "Pico accepts a managed generic Type specialization with scalar fields, construction and methods")
Check(picoGenericTypeUnit.content.Contains("BMXEmbeddedObject object;") And picoGenericTypeUnit.content.Contains("const BMXEmbeddedTypeDescriptor") And picoGenericTypeUnit.content.Contains("bmx_embedded_object_allocate(") And picoGenericTypeUnit.content.Contains("BMX_EMBEDDED_ROOT_OBJECT"), "Pico managed generic Type units publish the embedded object descriptor and root allocation during construction")
Local picoGenericOrdinaryAbiSource:String = "SuperStrict~nStruct SCount~nField value:Int~nEnd Struct~nType THelper~nMethod Read:Int()~nReturn 7~nEnd Method~nEnd Type~nType TBox<T>~nField data:Byte Ptr~nField pointers:Byte Ptr[]~nMethod Measure:SCount(helper:THelper)~nFunction Positive:Int(value:Int) Inline~nReturn value>0~nEnd Function~nLocal count:SCount~ncount.value=helper.Read()~nIf Positive(count.value) Then Return count~nReturn count~nEnd Method~nEnd Type~nGlobal box:TBox<Int>=New TBox<Int>~nGlobal count:SCount=box.Measure(New THelper)"
Local picoGenericOrdinaryAbi:TCompilerResult = TBlitzMaxCompiler.Compile("pico-generic-ordinary-abi.bmx", picoGenericOrdinaryAbiSource, noRuntimeResolver, noRuntimeOptions)
Local picoGenericOrdinaryAbiDiagnostics:TCompilerDiagnostic[]
Local picoGenericOrdinaryAbiBuild:TCompilerBuildOutputPlan = TBlitzMaxCompiler.PlanBuildOutputs(picoGenericOrdinaryAbi, "application.c", "", "", picoGenericOrdinaryAbiDiagnostics)
Local picoGenericOrdinaryAbiUnit:TCompilerBuildOutputFile
Local picoGenericOrdinaryAbiHeader:TCompilerBuildOutputFile
For Local file:TCompilerBuildOutputFile = EachIn picoGenericOrdinaryAbiBuild.files
	If file.role = "generic-specialization-c" Then picoGenericOrdinaryAbiUnit = file
	If file.role = "runtime-header" Then picoGenericOrdinaryAbiHeader = file
Next
Check(picoGenericOrdinaryAbi.Succeeded() And picoGenericOrdinaryAbiDiagnostics.length = 0 And picoGenericOrdinaryAbiUnit And picoGenericOrdinaryAbiHeader, "Pico generic Types plan their ordinary Struct layout authority as a companion embedded header")
Check(picoGenericOrdinaryAbiHeader.content.Contains("struct bmx_direct_scount_") And picoGenericOrdinaryAbiHeader.content.Contains("int32_t") And Not picoGenericOrdinaryAbiHeader.content.Contains("BBINT"), "Pico runtime headers publish ordinary Struct values using embedded scalar ABI names")
Check(picoGenericOrdinaryAbiUnit.content.Contains("#include ~q../../application.h~q") And picoGenericOrdinaryAbiUnit.content.Contains("bmx_embedded_type_methods((void *)") And picoGenericOrdinaryAbiUnit.content.Contains(", 0, 1)") And Not picoGenericOrdinaryAbiUnit.content.Contains("->clas->vfns"), "Pico generic units import the shared Struct layout, emit local helpers, and dispatch ordinary Types through embedded method tables")
Check(picoGenericOrdinaryAbiUnit.content.Contains("int8_t *") And picoGenericOrdinaryAbiUnit.content.Contains("BMXEmbeddedArray *"), "Pico generic Type fields retain pointer and pointer-Array storage")
Local picoGenericContainerSource:String = "SuperStrict~nInterface IValue<T>~nMethod Get:T()~nEnd Interface~nType TValues<T> Implements IValue<T>~nField values:T[]~nMethod New(initial:T)~nvalues=New T[1]~nvalues[0]=initial~nEnd Method~nMethod Get:T()~nReturn values[0]~nEnd Method~nMethod Copy:T[]()~nReturn values[..]~nEnd Method~nEnd Type~nGlobal container:TValues<String>=New TValues<String>(~qready~q)~nGlobal contract:IValue<String>=container~nGlobal copied:String[]=container.Copy()~nGlobal first:String=contract.Get()"
Local picoGenericContainer:TCompilerResult = TBlitzMaxCompiler.Compile("pico-generic-container.bmx", picoGenericContainerSource, noRuntimeResolver, noRuntimeOptions)
Local picoGenericContainerDiagnostics:TCompilerDiagnostic[]
Local picoGenericContainerBuild:TCompilerBuildOutputPlan = TBlitzMaxCompiler.PlanBuildOutputs(picoGenericContainer, "application.c", "", "", picoGenericContainerDiagnostics)
Local picoGenericContainerUnits:String
Local picoGenericContainerApplication:TCompilerBuildOutputFile
For Local file:TCompilerBuildOutputFile = EachIn picoGenericContainerBuild.files
	If file.role = "generic-specialization-c" Then picoGenericContainerUnits :+ file.content
	If file.role = "application-c" Then picoGenericContainerApplication = file
Next
Check(picoGenericContainer.Succeeded() And picoGenericContainerDiagnostics.length = 0 And picoGenericContainerBuild.linkInputs.length = 2, "Pico plans canonical generic Type and Interface specializations as separate reusable units")
Check(picoGenericContainerUnits.Contains("BMXEmbeddedInterfaceDescriptor") And picoGenericContainerUnits.Contains("BMXEmbeddedInterfaceEntry") And picoGenericContainerUnits.Contains("bmx_embedded_array_new_1d") And picoGenericContainerUnits.Contains("bmx_embedded_array_element") And picoGenericContainerUnits.Contains("bmx_embedded_array_slice"), "Pico generic containers publish interface metadata and lower managed Array allocation, access and slicing through the embedded ABI")
Check(picoGenericContainerApplication And picoGenericContainerApplication.content.Contains("bmx_embedded_interface_methods("), "Pico application consumers dispatch imported canonical generic Interfaces through embedded descriptors")
Local picoNestedGenericContainerSource:String = "SuperStrict~nInterface IEquality<T>~nMethod Equal:Int(left:T,right:T)~nEnd Interface~nType TFailure~nEnd Type~nType TKey<T>~nField value:T~nMethod Cast:TKey<T>(other:Object)~nReturn TKey<T>(other)~nEnd Method~nEnd Type~nType TBucket<T>~nField keys:T[]~nField equality:IEquality<T>~nMethod New()~nkeys=New T[1]~nEnd Method~nMethod Failure:TFailure()~nReturn New TFailure~nEnd Method~nEnd Type~nGlobal bucket:TBucket<TKey<Int>>=New TBucket<TKey<Int>>~nGlobal key:TKey<Int>=New TKey<Int>~nGlobal castKey:TKey<Int>=key.Cast(key)~nGlobal failure:TFailure=bucket.Failure()"
Local picoNestedGenericContainer:TCompilerResult = TBlitzMaxCompiler.Compile("pico-nested-generic-container.bmx", picoNestedGenericContainerSource, noRuntimeResolver, noRuntimeOptions)
Local picoNestedGenericContainerDiagnostics:TCompilerDiagnostic[]
Local picoNestedGenericContainerBuild:TCompilerBuildOutputPlan = TBlitzMaxCompiler.PlanBuildOutputs(picoNestedGenericContainer, "application.c", "", "", picoNestedGenericContainerDiagnostics)
Local picoNestedGenericContainerUnits:String
For Local file:TCompilerBuildOutputFile = EachIn picoNestedGenericContainerBuild.files
	If file.role = "generic-specialization-c" Then picoNestedGenericContainerUnits :+ file.content
Next
Check(picoNestedGenericContainer.Succeeded() And picoNestedGenericContainerDiagnostics.length = 0 And picoNestedGenericContainerBuild.linkInputs.length = 3, "Pico plans nested generic object Arrays and their referenced generic Interface as canonical units")
Check(picoNestedGenericContainerUnits.Contains("struct bmx_gen_"), "Pico nested generic units declare closed object layouts")
Check(picoNestedGenericContainerUnits.Contains("BMXEmbeddedInterfaceDescriptor"), "Pico nested generic units declare referenced Interface descriptors")
Check(picoNestedGenericContainerUnits.Contains("_New_ObjectNew(void);"), "Pico nested generic units declare ordinary zero-argument constructors")
Check(picoNestedGenericContainerUnits.Contains("bmx_embedded_object_cast(") And Not picoNestedGenericContainerUnits.Contains("bbObjectDowncast("), "Pico nested generic units lower explicit generic object casts through the embedded ABI")
Local picoGenericInheritanceSource:String = "SuperStrict~nInterface IValue<T>~nMethod Value:T()~nEnd Interface~nType TBase<T> Implements IValue<T>~nField baseValue:T~nMethod New(value:T)~nbaseValue=value~nEnd Method~nMethod Value:T()~nReturn baseValue~nEnd Method~nMethod Dispatch:T()~nReturn Value()~nEnd Method~nEnd Type~nType TDerived<T> Extends TBase<T>~nField derivedValue:T~nMethod New(baseValue:T,derivedValue:T)~nSuper.New(baseValue)~nSelf.derivedValue=derivedValue~nEnd Method~nMethod Value:T() Override~nReturn derivedValue~nEnd Method~nMethod OriginalValue:T()~nReturn Super.Value()~nEnd Method~nEnd Type~nGlobal derived:TDerived<String>=New TDerived<String>(~qbase~q,~qderived~q)~nGlobal forwarded:TDerived<String>=New TDerived<String>(~qforwarded~q)~nGlobal base:TBase<String>=derived~nGlobal contract:IValue<String>=derived~nGlobal dynamicValue:String=base.Value()~nGlobal selfDispatch:String=derived.Dispatch()~nGlobal originalValue:String=derived.OriginalValue()~nGlobal interfaceValue:String=contract.Value()"
Local picoGenericInheritance:TCompilerResult = TBlitzMaxCompiler.Compile("pico-generic-inheritance.bmx", picoGenericInheritanceSource, noRuntimeResolver, noRuntimeOptions)
Local picoGenericInheritanceDiagnostics:TCompilerDiagnostic[]
Local picoGenericInheritanceBuild:TCompilerBuildOutputPlan = TBlitzMaxCompiler.PlanBuildOutputs(picoGenericInheritance, "application.c", "", "", picoGenericInheritanceDiagnostics)
Local picoGenericInheritanceUnits:String
Local picoGenericInheritanceApplication:TCompilerBuildOutputFile
For Local file:TCompilerBuildOutputFile = EachIn picoGenericInheritanceBuild.files
	If file.role = "generic-specialization-c" Then picoGenericInheritanceUnits :+ file.content
	If file.role = "application-c" Then picoGenericInheritanceApplication = file
Next
Check(picoGenericInheritance.Succeeded() And picoGenericInheritanceDiagnostics.length = 0 And picoGenericInheritanceBuild.linkInputs.length = 3, "Pico plans a canonical generic Interface and base/derived Type hierarchy as three reusable units")
Check(picoGenericInheritanceUnits.Contains(".super = &bmx_gen_") And picoGenericInheritanceUnits.Contains(".string_count = 2") And picoGenericInheritanceUnits.Contains("bmx_embedded_new_root_slots[3]") And picoGenericInheritanceUnits.Contains("bmx_embedded_type_methods((void *)") And picoGenericInheritanceUnits.Contains("BMXEmbeddedInterfaceEntry"), "Pico derived generic descriptors flatten precise managed-field metadata, root managed constructor arguments, link their base descriptor, and retain virtual and Interface dispatch")
Check(picoGenericInheritanceUnits.Contains("bmx_call_receiver_materialized_receiver_") And Not picoGenericInheritanceUnits.Contains("bmx_array_materialized_receiver_"), "Pico generic virtual calls root materialized Object receivers under their declared temporary identity")
Check(picoGenericInheritanceApplication And Occurrences(picoGenericInheritanceApplication.content, "bmx_embedded_type_methods((void *)") >= 3 And picoGenericInheritanceApplication.content.Contains("bmx_embedded_interface_methods((void *)"), "Pico application consumers use descriptor slots for base, derived, Self-dispatched and Interface generic calls")
Local picoGenericFinalizerSource:String = "SuperStrict~nGlobal baseFinalized:UInt~nGlobal derivedFinalized:UInt~nType TFinalBase<T>~nMethod Delete()~nbaseFinalized:+1~nEnd Method~nEnd Type~nType TFinalDerived<T> Extends TFinalBase<T>~nMethod Delete()~nderivedFinalized:+1~nEnd Method~nEnd Type~nType TInheritedFinal<T> Extends TFinalBase<T>~nEnd Type~nGlobal base:TFinalBase<Int>=New TFinalBase<Int>~nGlobal derived:TFinalDerived<Int>=New TFinalDerived<Int>~nGlobal inherited:TInheritedFinal<Int>=New TInheritedFinal<Int>"
Local picoGenericFinalizer:TCompilerResult = TBlitzMaxCompiler.Compile("pico-generic-finalizer.bmx", picoGenericFinalizerSource, noRuntimeResolver, noRuntimeOptions)
Local picoGenericFinalizerDiagnostics:TCompilerDiagnostic[]
Local picoGenericFinalizerBuild:TCompilerBuildOutputPlan = TBlitzMaxCompiler.PlanBuildOutputs(picoGenericFinalizer, "application.c", "", "", picoGenericFinalizerDiagnostics)
Local picoGenericFinalizerUnits:String
For Local file:TCompilerBuildOutputFile = EachIn picoGenericFinalizerBuild.files
	If file.role = "generic-specialization-c" Then picoGenericFinalizerUnits :+ file.content
Next
Check(picoGenericFinalizer.Succeeded() And picoGenericFinalizerDiagnostics.length = 0 And picoGenericFinalizerBuild.linkInputs.length = 3, "Pico plans own, overridden and inherited generic Type finalizers as three canonical units")
Check(Occurrences(picoGenericFinalizerUnits, "BMX_EMBEDDED_TYPE_FLAG_HAS_FINALIZER") = 3 And Occurrences(picoGenericFinalizerUnits, ".finalizer = bmx_gen_") = 3 And Occurrences(picoGenericFinalizerUnits, "_finalize(object);") = 2 And Occurrences(picoGenericFinalizerUnits, "_Delete((struct") = 2 And picoGenericFinalizerUnits.Contains("extern uint32_t"), "Pico generic descriptors publish finalizers, declare ordinary dependencies, chain base Delete bodies, and do not synthesize a second inherited Delete call")
Local picoGenericObjectHooksSource:String = "SuperStrict~nGlobal baseCalls:UInt~nGlobal derivedCalls:UInt~nType THookBase<T>~nMethod ToString:String() Override~nbaseCalls:+1~nReturn ~qbase~q~nEnd Method~nMethod Compare:Int(other:Object)~nbaseCalls:+1~nReturn -10~nEnd Method~nMethod HashCode:UInt()~nbaseCalls:+1~nReturn 100~nEnd Method~nMethod Equals:Int(other:Object)~nbaseCalls:+1~nReturn True~nEnd Method~nEnd Type~nType THookDerived<T> Extends THookBase<T>~nMethod ToString:String() Override~nderivedCalls:+1~nReturn ~qderived~q~nEnd Method~nMethod Compare:Int(other:Object) Override~nderivedCalls:+1~nReturn -20~nEnd Method~nMethod HashCode:UInt() Override~nderivedCalls:+1~nReturn 200~nEnd Method~nMethod Equals:Int(other:Object) Override~nderivedCalls:+1~nReturn False~nEnd Method~nEnd Type~nType THookInherited<T> Extends THookBase<T>~nEnd Type~nGlobal base:Object=New THookBase<Int>~nGlobal derived:Object=New THookDerived<Int>~nGlobal inherited:Object=New THookInherited<Int>~nGlobal baseCompare:Int=base.Compare(derived)~nGlobal baseHash:UInt=base.HashCode()~nGlobal baseEquals:Int=base.Equals(derived)~nGlobal derivedCompare:Int=derived.Compare(base)~nGlobal derivedHash:UInt=derived.HashCode()~nGlobal derivedEquals:Int=derived.Equals(base)~nGlobal inheritedCompare:Int=inherited.Compare(base)~nGlobal inheritedHash:UInt=inherited.HashCode()~nGlobal inheritedEquals:Int=inherited.Equals(base)"
Local picoGenericObjectHooks:TCompilerResult = TBlitzMaxCompiler.Compile("pico-generic-object-hooks.bmx", picoGenericObjectHooksSource, noRuntimeResolver, noRuntimeOptions)
Local picoGenericObjectHooksDiagnostics:TCompilerDiagnostic[]
Local picoGenericObjectHooksBuild:TCompilerBuildOutputPlan = TBlitzMaxCompiler.PlanBuildOutputs(picoGenericObjectHooks, "application.c", "", "", picoGenericObjectHooksDiagnostics)
Local picoGenericObjectHookUnits:String
Local picoGenericObjectHookApplication:TCompilerBuildOutputFile
For Local file:TCompilerBuildOutputFile = EachIn picoGenericObjectHooksBuild.files
	If file.role = "generic-specialization-c" Then picoGenericObjectHookUnits :+ file.content
	If file.role = "application-c" Then picoGenericObjectHookApplication = file
Next
Check(picoGenericObjectHooks.Succeeded() And picoGenericObjectHooksDiagnostics.length = 0 And picoGenericObjectHooksBuild.linkInputs.length = 3, "Pico plans generic Types with own, overridden and inherited Object hooks as three canonical units")
Check(Occurrences(picoGenericObjectHookUnits, ".to_string = bmx_gen_") = 3 And Occurrences(picoGenericObjectHookUnits, ".compare = bmx_gen_") = 3 And Occurrences(picoGenericObjectHookUnits, ".hash_code = bmx_gen_") = 3 And Occurrences(picoGenericObjectHookUnits, ".equals = bmx_gen_") = 3 And Occurrences(picoGenericObjectHookUnits, "static const BMXEmbeddedString *bmx_gen_") >= 3 And Occurrences(picoGenericObjectHookUnits, "static int32_t bmx_gen_") >= 6 And Occurrences(picoGenericObjectHookUnits, "static uint32_t bmx_gen_") >= 3 And picoGenericObjectHookUnits.Contains("extern uint32_t"), "Pico generic descriptors publish signature-matched Object hooks and wrappers for inherited and overridden implementations")
Check(picoGenericObjectHookApplication And Occurrences(picoGenericObjectHookApplication.content, "bmx_embedded_object_compare(") = 3 And Occurrences(picoGenericObjectHookApplication.content, "bmx_embedded_object_hash_code(") = 3 And Occurrences(picoGenericObjectHookApplication.content, "bmx_embedded_object_equals(") = 3, "Pico application consumers dispatch generic Compare, HashCode and Equals calls through descriptor hooks")
Local picoString:TCompilerResult = TBlitzMaxCompiler.Compile("pico-string.bmx", "SuperStrict~nLocal greeting:String = ~qhello~q~nLocal length:Int = greeting.Length~nLocal matches:Int = greeting = ~qhello~q", noRuntimeResolver, noRuntimeOptions)
Local picoStringDiagnostics:TCompilerDiagnostic[]
Local picoStringC:String = TBlitzMaxCompiler.EmitC(picoString, picoStringDiagnostics)
Check(picoString.Succeeded() And picoStringDiagnostics.length = 0 And Contains(picoStringC, "#include <blitzmax/embedded_runtime.h>") And Contains(picoStringC, "static const BMXEmbeddedString bmx_embedded_string_str0") And Contains(picoStringC, "->length") And Contains(picoStringC, "bmx_embedded_string_equals("), "Pico standalone C emits immutable embedded String literals, length access and comparison through the embedded runtime ABI")
Local picoStringMethods:TCompilerResult = TBlitzMaxCompiler.Compile("pico-string-methods.bmx", "SuperStrict~nLocal value:String = ~qalpha~q~nLocal same:String = value.ToString()~nLocal ordering:Int = value.Compare(~qbeta~q)~nLocal equal:Int = value.Equals(same)~nLocal hash:UInt = value.HashCode()", noRuntimeResolver, noRuntimeOptions)
Local picoStringMethodDiagnostics:TCompilerDiagnostic[]
Local picoStringMethodC:String = TBlitzMaxCompiler.EmitC(picoStringMethods, picoStringMethodDiagnostics)
Check(picoStringMethods.Succeeded() And picoStringMethodDiagnostics.length = 0 And Contains(picoStringMethodC, "bmx_embedded_string_to_string(") And Contains(picoStringMethodC, "bmx_embedded_string_compare(") And Contains(picoStringMethodC, "bmx_embedded_string_equals(") And Contains(picoStringMethodC, "bmx_embedded_string_hash(") And Not Contains(picoStringMethodC, "extern const BMXEmbeddedString * bbString"), "Pico String value methods lower directly to the compact embedded runtime ABI")
Local picoStringCaseMethods:TCompilerResult = TBlitzMaxCompiler.Compile("pico-string-case-methods.bmx", "SuperStrict~nLocal value:String = ~qalpha~q~nLocal ordering:Int = value.Compare(~qBETA~q, False)~nLocal equal:Int = value.Equals(~qALPHA~q, False)~nLocal hash:UInt = value.HashCode(False)", noRuntimeResolver, noRuntimeOptions)
Local picoStringCaseMethodDiagnostics:TCompilerDiagnostic[]
Local picoStringCaseMethodC:String = TBlitzMaxCompiler.EmitC(picoStringCaseMethods, picoStringCaseMethodDiagnostics)
Check(picoStringCaseMethods.Succeeded() And picoStringCaseMethodDiagnostics.length = 0 And Contains(picoStringCaseMethodC, "bmx_embedded_string_compare_case(") And Contains(picoStringCaseMethodC, "bmx_embedded_string_equals_case(") And Contains(picoStringCaseMethodC, "bmx_embedded_string_hash_case(") And Not Contains(picoStringCaseMethodC, "bbStringCompareCase") And Not Contains(picoStringCaseMethodC, "bbStringEqualsCase") And Not Contains(picoStringCaseMethodC, "bbStringHashCase"), "Pico case-aware String overloads lower directly to the optional-capability embedded runtime ABI")
Local picoDynamicString:TCompilerResult = TBlitzMaxCompiler.Compile("pico-dynamic-string.bmx", "SuperStrict~nLocal punctuation:Int = 33~nLocal greeting:String = (~qhello~q + ~q ~q) + (~qworld~q + Chr(punctuation))~nLocal word:String = greeting[6..11]", noRuntimeResolver, noRuntimeOptions)
Local picoDynamicStringDiagnostics:TCompilerDiagnostic[]
Local picoDynamicStringC:String = TBlitzMaxCompiler.EmitC(picoDynamicString, picoDynamicStringDiagnostics)
Check(picoDynamicString.Succeeded() And picoDynamicStringDiagnostics.length = 0 And Occurrences(picoDynamicStringC, "bmx_embedded_string_concat(") = 3 And Contains(picoDynamicStringC, "bmx_embedded_string_slice(") And Contains(picoDynamicStringC, "bmx_embedded_string_from_char(") And Contains(picoDynamicStringC, "BMX_EMBEDDED_ROOT_STRING") And Contains(picoDynamicStringC, "bmx_tmp_"), "Pico dynamic String operations lower through rooted, explicitly sequenced typed IR temporaries")
Local picoEnum:TCompilerResult = TBlitzMaxCompiler.Compile("pico-enum.bmx", "SuperStrict~nEnum EMode:Byte~nIdle=2~nActive=7~nEnd Enum~nLocal mode:EMode=EMode.Active~nLocal ordinal:Byte=mode.Ordinal()~nLocal values:EMode[]=New EMode[2]~nvalues[0]=mode~nLocal total:Int~nFor Local value:EMode=EachIn values~ntotal:+Int(value)~nNext", noRuntimeResolver, noRuntimeOptions)
Local picoEnumDiagnostics:TCompilerDiagnostic[]
Local picoEnumC:String = TBlitzMaxCompiler.EmitC(picoEnum, picoEnumDiagnostics)
Check(picoEnum.Succeeded() And picoEnumDiagnostics.length = 0 And Contains(picoEnumC, "uint8_t bmx_v0_mode = 7") And Contains(picoEnumC, "BMX_EMBEDDED_ARRAY_ELEMENT_VALUE, 0") And Contains(picoEnumC, "bmx_embedded_array_element("), "Pico scalar Enums retain their underlying representation, Ordinal operation, Arrays, indexing and iteration")
Local picoEnumReflection:TCompilerResult = TBlitzMaxCompiler.Compile("pico-enum-reflection.bmx", "SuperStrict~nEnum EMode~nIdle=2~nActive=7~nEnd Enum~nLocal text:String=EMode.Idle.ToString()~nLocal values:EMode[]=EMode.Values()~nLocal converted:EMode~nLocal ok:Int=EMode.TryConvert(7,converted)~nLocal parsed:EMode=EMode.FromString(~qactive~q)", noRuntimeResolver, noRuntimeOptions)
Local picoEnumReflectionDiagnostics:TCompilerDiagnostic[]
Local picoEnumReflectionC:String = TBlitzMaxCompiler.EmitC(picoEnumReflection, picoEnumReflectionDiagnostics)
Check(picoEnumReflection.Succeeded() And picoEnumReflectionDiagnostics.length = 0 And Contains(picoEnumReflectionC, "static const BMXEmbeddedEnumDescriptor") And Contains(picoEnumReflectionC, "bmx_embedded_enum_to_string(") And Contains(picoEnumReflectionC, "bmx_embedded_enum_values(") And Contains(picoEnumReflectionC, "bmx_embedded_enum_try_convert(") And Contains(picoEnumReflectionC, "bmx_embedded_enum_from_string("), "Pico local Enums emit compact flash descriptors for names, Values, TryConvert and case-insensitive parsing")
Local picoStruct:TCompilerResult = TBlitzMaxCompiler.Compile("pico-struct.bmx", "SuperStrict~nStruct SPoint~nField x:Int=3~nField y:Int~nMethod New(x:Int,y:Int)~nSelf.x=x~nSelf.y=y~nEnd Method~nMethod Sum:Int()~nReturn x+y~nEnd Method~nEnd Struct~nStruct SBounds~nField first:SPoint~nField last:SPoint~nEnd Struct~nType THolder~nField point:SPoint~nEnd Type~nLocal point:SPoint=New SPoint(19,23)~nLocal bounds:SBounds~nbounds.first=point~nLocal points:SPoint[]=New SPoint[2]~npoints[1]=point~nLocal holder:THolder=New THolder~nholder.point=points[1]~nLocal total:Int=holder.point.Sum()", noRuntimeResolver, noRuntimeOptions)
Local picoStructDiagnostics:TCompilerDiagnostic[]
Local picoStructC:String = TBlitzMaxCompiler.EmitC(picoStruct, picoStructDiagnostics)
Check(picoStruct.Succeeded() And picoStructDiagnostics.length = 0 And Contains(picoStructC, "struct bmx_struct_") And Contains(picoStructC, "bmx_struct_new_") And Contains(picoStructC, "bmx_struct_array_init_") And Contains(picoStructC, "BMX_EMBEDDED_ARRAY_ELEMENT_VALUE, bmx_struct_array_init_") And Contains(picoStructC, "bmx_embedded_array_element("), "Pico unmanaged Structs support construction, methods, nesting, value copy, Object fields and initialized one-dimensional Arrays")
Local picoManagedStruct:TCompilerResult = TBlitzMaxCompiler.Compile("pico-managed-struct.bmx", "SuperStrict~nType TNode~nEnd Type~nStruct SText~nField value:String=~qroot~q~nField items:Int[]~nField node:TNode~nField aliases:String[2]~nEnd Struct~nStruct SNested~nField inner:SText~nEnd Struct~nType THolder~nField state:SNested~nEnd Type~nLocal value:SText~nLocal values:SText[]=New SText[2]~nLocal holder:THolder=New THolder~nvalues[0]=value~nholder.state.inner=values[0]", noRuntimeResolver, noRuntimeOptions)
Local picoManagedStructDiagnostics:TCompilerDiagnostic[]
Local picoManagedStructC:String = TBlitzMaxCompiler.EmitC(picoManagedStruct, picoManagedStructDiagnostics)
Check(picoManagedStruct.Succeeded() And picoManagedStructDiagnostics.length = 0, "Pico managed Structs compile through the recursive precise descriptor tier")
Local picoArray:TCompilerResult = TBlitzMaxCompiler.Compile("pico-array.bmx", "SuperStrict~nLocal values:Int[] = New Int[4]~nvalues[0] = 7~nLocal length:Int = values.Length~nLocal first:Int = values[0]~nLocal total:Int~nFor Local value:Int = EachIn values~ntotal :+ value~nNext", noRuntimeResolver, noRuntimeOptions)
Local picoArrayDiagnostics:TCompilerDiagnostic[]
Local picoArrayC:String = TBlitzMaxCompiler.EmitC(picoArray, picoArrayDiagnostics)
Check(picoArray.Succeeded() And picoArrayDiagnostics.length = 0 And Contains(picoArrayC, "BMXEmbeddedArray *") And Contains(picoArrayC, "bmx_embedded_array_new_1d(") And Contains(picoArrayC, "bmx_embedded_array_element(") And Contains(picoArrayC, "->length") And Contains(picoArrayC, "BMXEmbeddedRootSlot bmx_embedded_root_slots") And Contains(picoArrayC, "BMX_EMBEDDED_ROOT_ARRAY"), "Pico standalone C emits arena-backed scalar Arrays with typed roots even when the module declares no Object Types")
Local picoArrayConcatSource:String = "SuperStrict~nLocal numbers:Int[]=[1,2,3]~nLocal first:String[]=[~qa~q,~qb~q]~nLocal second:String[]=[~qc~q]~nLocal joined:String[]=first+second~njoined:+[~qd~q]"
Local picoArrayConcat:TCompilerResult = TBlitzMaxCompiler.Compile("pico-array-concat.bmx", picoArrayConcatSource, noRuntimeResolver, noRuntimeOptions)
Local picoArrayConcatDiagnostics:TCompilerDiagnostic[]
Local picoArrayConcatC:String = TBlitzMaxCompiler.EmitC(picoArrayConcat, picoArrayConcatDiagnostics)
Check(picoArrayConcat.Succeeded() And picoArrayConcatDiagnostics.length = 0 And Occurrences(picoArrayConcatC, "bmx_embedded_array_from_data(") = 4 And Occurrences(picoArrayConcatC, "bmx_embedded_array_concat(") = 2, "Pico Array literals, concatenation and :+ assignment lower through the embedded managed-container ABI: " + CompilerDiagnosticSummary(picoArrayConcat) + "; emission: " + EmissionDiagnosticSummary(picoArrayConcatDiagnostics))
Check(Contains(picoArrayConcatC, "BMX_EMBEDDED_ARRAY_ELEMENT_VALUE") And Contains(picoArrayConcatC, "BMX_EMBEDDED_ARRAY_ELEMENT_STRING") And Contains(picoArrayConcatC, "BMX_EMBEDDED_ROOT_STRING") And Contains(picoArrayConcatC, "BMX_EMBEDDED_ROOT_ARRAY") And Contains(picoArrayConcatC, "bmx_tmp_"), "Pico Array literals and concatenation preserve precise roots for materialized elements and the sequenced left operand")
Local picoBlockMemory:TCompilerResult = TBlitzMaxCompiler.Compile("pico-block-memory.bmx", "SuperStrict~nLocal source:Byte[]=New Byte[16]~nLocal destination:Byte[]=New Byte[16]~nMemCopy(destination,source,16)~nLocal memory:Byte Ptr=MemAlloc(16)~nMemClear(memory,16)~nMemCopy(memory,source,16)~nMemMove(memory+1,memory,8)~nmemory=MemExtend(memory,16,32)~nMemCopy(destination,memory,16)~nMemFree(memory)", noRuntimeResolver, noRuntimeOptions)
Local picoBlockMemoryDiagnostics:TCompilerDiagnostic[]
Local picoBlockMemoryC:String = TBlitzMaxCompiler.EmitC(picoBlockMemory, picoBlockMemoryDiagnostics)
Check(picoBlockMemory.Succeeded() And picoBlockMemoryDiagnostics.length = 0, "Pico exposes the standard BRL.Blitz block-memory surface implicitly")
Check(Occurrences(picoBlockMemoryC, "bmx_embedded_array_data(") >= 4 And Contains(picoBlockMemoryC, "brl_blitz_MemAlloc__Bsize_t__Bint") And Contains(picoBlockMemoryC, "brl_blitz_MemExtend__PBbyte__Bsize_t__Bsize_t__Bint") And Contains(picoBlockMemoryC, "brl_blitz_MemFree__PBbyte__Bint") And Contains(picoBlockMemoryC, "bbMemClear(") And Contains(picoBlockMemoryC, "bbMemCopy(") And Contains(picoBlockMemoryC, "bbMemMove("), "Pico lowers managed Array payloads and standard allocation, clearing, copying, moving, extension, and release calls through the embedded ABI")
Local picoReferenceArray:TCompilerResult = TBlitzMaxCompiler.Compile("pico-reference-array.bmx", "SuperStrict~nLocal values:String[] = New String[2]", noRuntimeResolver, noRuntimeOptions)
Local picoReferenceArrayDiagnostics:TCompilerDiagnostic[]
Local picoReferenceArrayC:String = TBlitzMaxCompiler.EmitC(picoReferenceArray, picoReferenceArrayDiagnostics)
Check(picoReferenceArray.Succeeded() And picoReferenceArrayDiagnostics.length = 0 And Contains(picoReferenceArrayC, "BMX_EMBEDDED_ARRAY_ELEMENT_STRING"), "Pico managed-container profile emits String Array element metadata")
Local picoObjectArray:TCompilerResult = TBlitzMaxCompiler.Compile("pico-object-array.bmx", "SuperStrict~nType TItem~nField value:Int~nEnd Type~nLocal values:TItem[] = New TItem[2]~nvalues[0] = New TItem~nLocal first:TItem = values[0]", noRuntimeResolver, noRuntimeOptions)
Local picoObjectArrayDiagnostics:TCompilerDiagnostic[]
Local picoObjectArrayC:String = TBlitzMaxCompiler.EmitC(picoObjectArray, picoObjectArrayDiagnostics)
Check(picoObjectArray.Succeeded() And picoObjectArrayDiagnostics.length = 0 And Contains(picoObjectArrayC, "BMX_EMBEDDED_ARRAY_ELEMENT_OBJECT") And Contains(picoObjectArrayC, "BMXEmbeddedRootSlot bmx_embedded_root_slots") And Contains(picoObjectArrayC, "BMX_EMBEDDED_ROOT_ARRAY"), "Pico Object Arrays emit precise element metadata and typed Array roots")
Local picoObjectSource:String = "SuperStrict~nType TCounter~nField value:Int~nMethod New(initial:Int)~nvalue = initial~nEnd Method~nMethod Add:Int(delta:Int)~nvalue :+ delta~nReturn value~nEnd Method~nMethod ToString:String() Override~nReturn ~qcounter~q~nEnd Method~nEnd Type~nLocal counter:TCounter = New TCounter(4)~nLocal result:Int = counter.Add(3)"
Local picoObject:TCompilerResult = TBlitzMaxCompiler.Compile("pico-object.bmx", picoObjectSource, noRuntimeResolver, noRuntimeOptions)
Local picoObjectDiagnostics:TCompilerDiagnostic[]
Local picoObjectC:String = TBlitzMaxCompiler.EmitC(picoObject, picoObjectDiagnostics)
Check(picoObject.Succeeded() And picoObjectDiagnostics.length = 0 And Contains(picoObjectC, "BMXEmbeddedObject object;") And Contains(picoObjectC, "static const BMXEmbeddedTypeDescriptor bmx_embedded_type_") And Contains(picoObjectC, "bmx_embedded_object_allocate(&bmx_embedded_type_") And Contains(picoObjectC, "bmx_embedded_object_not_null(") And Contains(picoObjectC, "BMXEmbeddedRootFrame bmx_embedded_root_frame;") And Contains(picoObjectC, "bmx_embedded_root_frame_enter(") And Contains(picoObjectC, "bmx_embedded_root_frame_leave(") And Contains(picoObjectC, "bmx_fn1_Add(") And Contains(picoObjectC, ".to_string = bmx_embedded_to_string_") And Contains(picoObjectC, "static const BMXEmbeddedString *bmx_embedded_to_string_"), "Pico standalone C emits described arena-backed concrete Objects with safepoint-driven root frames, diagnostic ToString hooks, cheap release receiver checks, constructors, fields and virtual method calls")
Local picoDebugObjectOptions:TCompilerOptions = TestOptions()
picoDebugObjectOptions.targetPlatform = "pico"
picoDebugObjectOptions.targetArchitecture = "arm"
picoDebugObjectOptions.threaded = False
picoDebugObjectOptions.implicitRuntime = False
picoDebugObjectOptions.buildMode = "debug"
picoDebugObjectOptions.gdbDebug = True
picoDebugObjectOptions.RefreshConditionalSymbols()
Local picoDebugObject:TCompilerResult = TBlitzMaxCompiler.Compile("pico-debug-object.bmx", picoObjectSource, noRuntimeResolver, picoDebugObjectOptions)
Local picoDebugObjectDiagnostics:TCompilerDiagnostic[]
Local picoDebugObjectC:String = TBlitzMaxCompiler.EmitC(picoDebugObject, picoDebugObjectDiagnostics)
Check(picoDebugObject.Succeeded() And picoDebugObjectDiagnostics.length = 0 And Contains(picoDebugObjectC, "bmx_embedded_object_assert(") And Contains(picoDebugObjectC, "bmx_embedded_root_frame_enter("), "Pico debug builds retain full heap/descriptor receiver validation and conservative root frames: " + CompilerDiagnosticSummary(picoDebugObject) + "; emission: " + EmissionDiagnosticSummary(picoDebugObjectDiagnostics))
Local picoFinalCallSource:String = "SuperStrict~nType TFinalCounter Final~nField value:Int~nMethod Step:Int(delta:Int)~nvalue:+delta~nReturn value~nEnd Method~nEnd Type~nLocal counter:TFinalCounter=New TFinalCounter~nLocal result:Int=counter.Step(3)"
Local picoFinalCall:TCompilerResult = TBlitzMaxCompiler.Compile("pico-final-call.bmx", picoFinalCallSource, noRuntimeResolver, noRuntimeOptions)
Local picoFinalCallDiagnostics:TCompilerDiagnostic[]
Local picoFinalCallC:String = TBlitzMaxCompiler.EmitC(picoFinalCall, picoFinalCallDiagnostics)
Check(picoFinalCall.Succeeded() And picoFinalCallDiagnostics.length = 0 And Contains(picoFinalCallC, "bmx_embedded_object_not_null(") And Not Contains(picoFinalCallC, ")->type->methods["), "Pico calls on a statically Final receiver devirtualize to its exact implementation while retaining the release null check")
Local desktopFinalCall:TCompilerResult = TBlitzMaxCompiler.Compile("desktop-final-call.bmx", picoFinalCallSource, resolver, TestOptions())
Local desktopFinalCallDiagnostics:TCompilerDiagnostic[]
Local desktopFinalCallC:String = TBlitzMaxCompiler.EmitRuntimeC(desktopFinalCall, desktopFinalCallDiagnostics)
Check(desktopFinalCall.Succeeded() And desktopFinalCallDiagnostics.length = 0 And Contains(desktopFinalCallC, "->clas->"), "Pico exact-receiver dispatch does not alter the established desktop class-table call path")
Local picoInheritance:TCompilerResult = TBlitzMaxCompiler.Compile("pico-inheritance.bmx", "SuperStrict~nType TBase Abstract~nField baseValue:Int=20~nMethod Score:Int(value:Int)~nReturn baseValue+value~nEnd Method~nMethod Stable:Int()~nReturn baseValue+1~nEnd Method~nMethod Delete()~nEnd Method~nEnd Type~nType TDerived Extends TBase~nField extra:Int=2~nMethod Score:Int(value:Int) Override~nReturn Super.Score(value)+extra~nEnd Method~nMethod Delete()~nEnd Method~nEnd Type~nLocal concrete:TDerived=New TDerived~nLocal base:TBase=concrete~nLocal result:Int=base.Score(20)+concrete.Stable()", noRuntimeResolver, noRuntimeOptions)
Local picoInheritanceDiagnostics:TCompilerDiagnostic[]
Local picoInheritanceC:String = TBlitzMaxCompiler.EmitC(picoInheritance, picoInheritanceDiagnostics)
Check(picoInheritance.Succeeded() And picoInheritanceDiagnostics.length = 0 And Contains(picoInheritanceC, ".super = &bmx_embedded_type_") And Contains(picoInheritanceC, "static const BMXEmbeddedMethod") And Contains(picoInheritanceC, "->type->methods[") And Contains(picoInheritanceC, "bmx_fn0_Score(") And Contains(picoInheritanceC, "bmx_fn3_Score(") And Contains(picoInheritanceC, "bmx_embedded_finalize_cls0_TBase(object);") And Contains(picoInheritanceC, "bmx_embedded_finalize_cls1_TDerived"), "Pico local inheritance supports abstract bases and emits base-first layouts, compact method tables, virtual override dispatch, direct Super calls and derived-to-base finalizer wrappers")
Local picoObjectSuperSource:String = "SuperStrict~nType TObjectBase~nMethod ToString:String() Override~nReturn ~qbase~q~nEnd Method~nMethod Compare:Int(other:Object) Override~nReturn 10~nEnd Method~nMethod HashCode:UInt() Override~nReturn 20~nEnd Method~nMethod Equals:Int(other:Object) Override~nReturn Self=other~nEnd Method~nEnd Type~nType TObjectDerived Extends TObjectBase~nMethod ToString:String() Override~nReturn Super.ToString()+~q-derived~q~nEnd Method~nMethod Compare:Int(other:Object) Override~nReturn Super.Compare(other)+1~nEnd Method~nMethod HashCode:UInt() Override~nReturn Super.HashCode()+1~nEnd Method~nMethod Equals:Int(other:Object) Override~nReturn Super.Equals(other)~nEnd Method~nEnd Type~nLocal value:TObjectDerived=New TObjectDerived~nLocal text:String=value.ToString()~nLocal ordering:Int=value.Compare(value)~nLocal hash:UInt=value.HashCode()~nLocal equal:Int=value.Equals(value)"
Local picoObjectSuper:TCompilerResult = TBlitzMaxCompiler.Compile("pico-object-super.bmx", picoObjectSuperSource, noRuntimeResolver, noRuntimeOptions)
Local picoObjectSuperDiagnostics:TCompilerDiagnostic[]
Local picoObjectSuperC:String = TBlitzMaxCompiler.EmitC(picoObjectSuper, picoObjectSuperDiagnostics)
Check(picoObjectSuper.Succeeded() And picoObjectSuperDiagnostics.length = 0 And Contains(picoObjectSuperC, "bmx_embedded_to_string_cls0_TObjectBase((void *)") And Contains(picoObjectSuperC, "bmx_embedded_compare_cls0_TObjectBase((void *)") And Contains(picoObjectSuperC, "bmx_embedded_hash_code_cls0_TObjectBase((void *)") And Contains(picoObjectSuperC, "bmx_embedded_equals_cls0_TObjectBase((void *)"), "Pico built-in Object Super calls dispatch directly through the base Type's compact value-operation hooks: " + CompilerDiagnosticSummary(picoObjectSuper) + "; emission: " + EmissionDiagnosticSummary(picoObjectSuperDiagnostics))
Local picoObjectSendMessageSource:String = "SuperStrict~nType TMessageBase~nMethod SendMessage:Object(message:Object,source:Object) Override~nReturn message~nEnd Method~nEnd Type~nType TMessageDerived Extends TMessageBase~nMethod SendMessage:Object(message:Object,source:Object) Override~nReturn Super.SendMessage(message,source)~nEnd Method~nEnd Type~nType TMessageDefault~nEnd Type~nLocal receiver:Object=New TMessageDerived~nLocal payload:Object=New TMessageDefault~nLocal result:Object=receiver.SendMessage(payload,Null)~nLocal missing:Object=(New TMessageDefault).SendMessage(payload,receiver)"
Local picoObjectSendMessage:TCompilerResult = TBlitzMaxCompiler.Compile("pico-object-send-message.bmx", picoObjectSendMessageSource, noRuntimeResolver, noRuntimeOptions)
Local picoObjectSendMessageDiagnostics:TCompilerDiagnostic[]
Local picoObjectSendMessageC:String = TBlitzMaxCompiler.EmitC(picoObjectSendMessage, picoObjectSendMessageDiagnostics)
Check(picoObjectSendMessage.Succeeded() And picoObjectSendMessageDiagnostics.length = 0 And Contains(picoObjectSendMessageC, ".send_message = bmx_embedded_send_message_cls1_TMessageDerived") And Contains(picoObjectSendMessageC, "bmx_embedded_send_message_cls0_TMessageBase((void *)") And Occurrences(picoObjectSendMessageC, "bmx_embedded_object_send_message((void *)") = 2, "Pico Object.SendMessage uses the compact override hook, direct Super hook and Null-returning runtime fallback: " + CompilerDiagnosticSummary(picoObjectSendMessage) + "; emission: " + EmissionDiagnosticSummary(picoObjectSendMessageDiagnostics))
Local picoGenericObjectSendMessage:TCompilerResult = TBlitzMaxCompiler.Compile("pico-generic-object-send-message.bmx", "SuperStrict~nType TGenericMessenger<T>~nMethod SendMessage:Object(message:Object,source:Object) Override~nReturn message~nEnd Method~nEnd Type~nLocal receiver:Object=New TGenericMessenger<Int>~nLocal payload:Object=New TGenericMessenger<Int>~nLocal result:Object=receiver.SendMessage(payload,Null)", noRuntimeResolver, noRuntimeOptions)
Local picoGenericObjectSendMessageDiagnostics:TCompilerDiagnostic[]
Local picoGenericObjectSendMessageBuild:TCompilerBuildOutputPlan = TBlitzMaxCompiler.PlanBuildOutputs(picoGenericObjectSendMessage, "application.c", "", "", picoGenericObjectSendMessageDiagnostics)
Local picoGenericObjectSendMessageUnits:String
For Local file:TCompilerBuildOutputFile = EachIn picoGenericObjectSendMessageBuild.files
	If file.role = "generic-specialization-c" Then picoGenericObjectSendMessageUnits :+ file.content
Next
Check(picoGenericObjectSendMessage.Succeeded() And picoGenericObjectSendMessageDiagnostics.length = 0 And Contains(picoGenericObjectSendMessageUnits, "_send_message(void *object, void *message, void *source)") And Contains(picoGenericObjectSendMessageUnits, ".send_message = "), "Pico generic Type specializations publish Object.SendMessage through their compact descriptors")
Local picoCallableReturnSource:String = "SuperStrict~nFunction Increment:Int(value:Int)~nReturn value+1~nEnd Function~nInterface IOperationFactory~nMethod Choose:Int(value:Int)(enabled:Int)~nEnd Interface~nType TOperationBase~nMethod Choose:Int(value:Int)(enabled:Int)~nIf enabled Then Return Increment~nReturn Null~nEnd Method~nEnd Type~nType TOperationDerived Extends TOperationBase Implements IOperationFactory~nMethod Choose:Int(value:Int)(enabled:Int) Override~nReturn Super.Choose(enabled)~nEnd Method~nEnd Type~nLocal concrete:TOperationDerived=New TOperationDerived~nLocal base:TOperationBase=concrete~nLocal factory:IOperationFactory=concrete~nLocal operation:Int(value:Int)=base.Choose(True)~nLocal first:Int=operation(40)~nLocal second:Int=factory.Choose(True)(41)"
Local picoCallableReturn:TCompilerResult = TBlitzMaxCompiler.Compile("pico-callable-return.bmx", picoCallableReturnSource, noRuntimeResolver, noRuntimeOptions)
Local picoCallableReturnDiagnostics:TCompilerDiagnostic[]
Local picoCallableReturnC:String = TBlitzMaxCompiler.EmitC(picoCallableReturn, picoCallableReturnDiagnostics)
Check(picoCallableReturn.Succeeded() And picoCallableReturnDiagnostics.length = 0 And Contains(picoCallableReturnC, "int32_t (*(*)(struct bmx_cls0_TOperationBase_obj *, int32_t))(int32_t)") And Contains(picoCallableReturnC, "int32_t (*(*)(BMXEmbeddedObject *, int32_t))(int32_t)"), "Pico virtual and Interface dispatch preserve nested C declarations for callable-returning methods: " + CompilerDiagnosticSummary(picoCallableReturn) + "; emission: " + EmissionDiagnosticSummary(picoCallableReturnDiagnostics))
Local picoInterface:TCompilerResult = TBlitzMaxCompiler.Compile("pico-interface.bmx", "SuperStrict~nInterface IScore~nMethod Score:Int(value:Int)~nEnd Interface~nInterface ILabel~nMethod Label:String()~nEnd Interface~nType TBase Implements IScore~nMethod Score:Int(value:Int)~nReturn value+1~nEnd Method~nEnd Type~nType TItem Extends TBase Implements ILabel~nMethod Score:Int(value:Int) Override~nReturn value+2~nEnd Method~nMethod Label:String()~nReturn ~qitem~q~nEnd Method~nEnd Type~nLocal item:TItem=New TItem~nLocal score:IScore=item~nLocal label:ILabel=item~nLocal value:Int=score.Score(40)~nLocal text:String=label.Label()~nLocal objectValue:Object=item~nLocal recovered:IScore=IScore(objectValue)", noRuntimeResolver, noRuntimeOptions)
Local picoInterfaceDiagnostics:TCompilerDiagnostic[]
Local picoInterfaceC:String = TBlitzMaxCompiler.EmitC(picoInterface, picoInterfaceDiagnostics)
Check(picoInterface.Succeeded() And picoInterfaceDiagnostics.length = 0 And Contains(picoInterfaceC, "static const BMXEmbeddedInterfaceDescriptor") And Contains(picoInterfaceC, "static const BMXEmbeddedInterfaceEntry") And Contains(picoInterfaceC, ".interfaces =") And Contains(picoInterfaceC, "bmx_embedded_interface_methods(") And Contains(picoInterfaceC, "bmx_embedded_interface_cast("), "Pico local Interfaces use erased Object references, compact per-Type tables, inherited overrides and checked casts")
Local picoImportedInterfaceSource:String = "SuperStrict~nImport pico.contracts~nType TReading Implements ILabelled~nMethod Read:Int(value:Int)~nReturn value+1~nEnd Method~nMethod Label:String()~nReturn ~qreading~q~nEnd Method~nEnd Type~nLocal concrete:TReading=New TReading~nLocal labelled:ILabelled=concrete~nLocal reading:IReading=IReading(labelled)~nLocal result:Int=reading.Read(41)~nTry~nThrow concrete~nCatch caught:ILabelled~nLocal text:String=caught.Label()~nEnd Try"
Local picoImportedInterface:TCompilerResult = TBlitzMaxCompiler.Compile("pico-imported-interface.bmx", picoImportedInterfaceSource, noRuntimeResolver, noRuntimeOptions)
Local picoImportedInterfaceDiagnostics:TCompilerDiagnostic[]
Local picoImportedInterfaceC:String = TBlitzMaxCompiler.EmitC(picoImportedInterface, picoImportedInterfaceDiagnostics)
Check(picoImportedInterface.Succeeded() And picoImportedInterfaceDiagnostics.length = 0, "Pico accepts source Types implementing arbitrary imported Interface hierarchies: " + CompilerDiagnosticSummary(picoImportedInterface))
Check(Contains(picoImportedInterfaceC, "{ ~qIReading~q, ~qpico_contracts_IReading~q }") And Contains(picoImportedInterfaceC, "{ ~qILabelled~q, ~qpico_contracts_ILabelled~q }") And Occurrences(picoImportedInterfaceC, "static const BMXEmbeddedInterfaceEntry") >= 1 And Contains(picoImportedInterfaceC, "bmx_embedded_interface_methods(") And Contains(picoImportedInterfaceC, "bmx_embedded_interface_cast(") And Contains(picoImportedInterfaceC, "BMX_EMBEDDED_EXCEPTION_OBJECT"), "Pico materializes imported Interface descriptors from stable published ABI names for inheritance, implementation, dispatch, casts, and catches")
Local picoImportedTypeSource:String = "SuperStrict~nImport pico.types~nLocal imported:TImportedReading=CreateImportedReading()~nLocal constructed:TImportedReading=New TImportedReading(40)~nLocal objectValue:Object=imported~nLocal recovered:TImportedReading=TImportedReading(objectValue)~nLocal result:Int=recovered.Read(41)+constructed.Read(2)~nTry~nThrow imported~nCatch caught:TImportedReading~nresult:+caught.Read(1)~nEnd Try"
Local picoImportedType:TCompilerResult = TBlitzMaxCompiler.Compile("pico-imported-type.bmx", picoImportedTypeSource, noRuntimeResolver, noRuntimeOptions)
Local picoImportedTypeDiagnostics:TCompilerDiagnostic[]
Local picoImportedTypeC:String = TBlitzMaxCompiler.EmitC(picoImportedType, picoImportedTypeDiagnostics)
Check(picoImportedType.Succeeded() And picoImportedTypeDiagnostics.length = 0, "Pico accepts imported concrete Types in parameters, returns, roots, casts, calls, throws, and catches: " + CompilerDiagnosticSummary(picoImportedType) + "; emission: " + EmissionDiagnosticSummary(picoImportedTypeDiagnostics))
Check(Contains(picoImportedTypeC, "struct pico_types_TImportedReading_obj;") And Contains(picoImportedTypeC, ".abi_name = ~qpico_types_TImportedReading~q") And Contains(picoImportedTypeC, "_pico_types_TImportedReading_New__Bint_ObjectNew(40)") And Contains(picoImportedTypeC, "((struct pico_types_TImportedReading_obj *)&bmx_embedded_null_object)") And Not Contains(picoImportedTypeC, "((struct pico_types_TImportedReading_obj *)&bbNullObject)") And Contains(picoImportedTypeC, "bmx_embedded_type_methods(") And Contains(picoImportedTypeC, "bmx_embedded_object_cast(") And Contains(picoImportedTypeC, "BMX_EMBEDDED_ROOT_OBJECT"), "Pico materializes stable imported Type identities, uses the embedded null sentinel, calls producer-owned construction helpers, and dispatches through the producer-owned compact method table")
Local picoImportedField:TCompilerResult = TBlitzMaxCompiler.Compile("pico-imported-field.bmx", "SuperStrict~nImport pico.types~nLocal reading:TImportedReading=CreateImportedReading()~nreading.value=41~nreading.label=~qfield~q~nLocal value:Int=reading.value~nLocal label:String=reading.label", noRuntimeResolver, noRuntimeOptions)
Local picoImportedFieldDiagnostics:TCompilerDiagnostic[]
Local picoImportedFieldC:String = TBlitzMaxCompiler.EmitC(picoImportedField, picoImportedFieldDiagnostics)
Check(picoImportedField.Succeeded() And picoImportedFieldDiagnostics.length = 0 And Contains(picoImportedFieldC, "struct pico_types_TImportedReading_obj {") And Contains(picoImportedFieldC, "BMXEmbeddedObject object;") And Contains(picoImportedFieldC, "int32_t _pico_types_timportedreading_value;") And Contains(picoImportedFieldC, "const BMXEmbeddedString * _pico_types_timportedreading_label;") And Contains(picoImportedFieldC, "->_pico_types_timportedreading_value") And Contains(picoImportedFieldC, "->_pico_types_timportedreading_label") And Contains(picoImportedFieldC, "bmx_embedded_object_not_null("), "Pico reconstructs the current imported Type layout for null-checked public scalar and managed field reads and writes")
Local picoImportedBaseSource:String = "SuperStrict~nImport pico.types~nType TLocalReading Extends TImportedReading~nField extra:Int=2~nField retained:String~nMethod New(initial:Int)~nSuper.New(initial)~nretained=~qlocal~q~nEnd Method~nMethod Read:Int(value:Int) Override~nReturn Super.Read(value)+extra~nEnd Method~nMethod Delete()~nEnd Method~nEnd Type~nLocal derived:TLocalReading=New TLocalReading(40)~nLocal base:TImportedReading=derived~nLocal stable:IStable=derived~nLocal result:Int=base.Read(0)+stable.Stable()+derived.value~nLocal recovered:TLocalReading=TLocalReading(base)"
Local picoImportedBase:TCompilerResult = TBlitzMaxCompiler.Compile("pico-imported-base.bmx", picoImportedBaseSource, noRuntimeResolver, noRuntimeOptions)
Local picoImportedBaseDiagnostics:TCompilerDiagnostic[]
Local picoImportedBaseC:String = TBlitzMaxCompiler.EmitC(picoImportedBase, picoImportedBaseDiagnostics)
Check(picoImportedBase.Succeeded() And picoImportedBaseDiagnostics.length = 0, "Pico accepts a local Type derived from an imported base: " + CompilerDiagnosticSummary(picoImportedBase) + "; emission: " + EmissionDiagnosticSummary(picoImportedBaseDiagnostics))
Check(AppearsBefore(picoImportedBaseC, "_pico_types_timportedreading_value", "bmx_field_f0_extra") And Contains(picoImportedBaseC, ".super = &bmx_embedded_imported_type_") And Contains(picoImportedBaseC, "_pico_types_TImportedReading_New__Bint((struct pico_types_TImportedReading_obj *)bmx_self_self, bmx_p0_initial);") And Contains(picoImportedBaseC, "_pico_types_TImportedReading_Read__Bint((struct pico_types_TImportedReading_obj *)bmx_self_self") And Contains(picoImportedBaseC, "interfaces_0[1] = {~n    (BMXEmbeddedMethod)_pico_types_TImportedReading_Stable") And Contains(picoImportedBaseC, "bmx_embedded_interface_methods") And Contains(picoImportedBaseC, "_pico_types_TImportedReading_EmbeddedBaseFinalize(object);") And Contains(picoImportedBaseC, "bmx_embedded_object_cast("), "Pico imported-base layouts preserve the base prefix and connect construction, inherited and overridden dispatch, inherited Interface implementations, Super calls, casts, and finalizer chaining")
Local picoImportedReceiver:TCompilerResult = TBlitzMaxCompiler.Compile("pico-imported-receiver.bmx", "SuperStrict~nImport pico.types~nType TLocalReading Extends TImportedReading~nMethod Probe:Int()~nReturn Read(1)~nEnd Method~nEnd Type~nLocal reading:TLocalReading=New TLocalReading~nLocal result:Int=reading.Probe()", noRuntimeResolver, noRuntimeOptions)
Local picoImportedReceiverDiagnostics:TCompilerDiagnostic[]
Local picoImportedReceiverC:String = TBlitzMaxCompiler.EmitC(picoImportedReceiver, picoImportedReceiverDiagnostics)
Check(picoImportedReceiver.Succeeded() And picoImportedReceiverDiagnostics.length = 0 And Contains(picoImportedReceiverC, ")((struct pico_types_TImportedReading_obj *)bmx_self_self"), "Pico virtual calls through an imported base cast derived receivers to the published method-table signature")
Local picoImportedTypeFunction:TCompilerResult = TBlitzMaxCompiler.Compile("pico-imported-type-function.bmx", "SuperStrict~nImport pico.typefunctions~nType TLocalFactory Extends TFactoryBase~nMethod Name:String() Override~nReturn ~qlocal~q~nEnd Method~nEnd Type~nLocal factory:TLocalFactory=New TLocalFactory", noRuntimeResolver, noRuntimeOptions)
Local picoImportedTypeFunctionDiagnostics:TCompilerDiagnostic[]
Local picoImportedTypeFunctionC:String = TBlitzMaxCompiler.EmitC(picoImportedTypeFunction, picoImportedTypeFunctionDiagnostics)
Check(picoImportedTypeFunction.Succeeded() And picoImportedTypeFunctionDiagnostics.length = 0 And AppearsBefore(picoImportedTypeFunctionC, "extern struct pico_typefunctions_TFactoryBase_obj * pico_typefunctions_TFactoryBase_Find__Bstring", "static const BMXEmbeddedMethod"), "Pico declares inherited imported Type functions before local derived descriptors reference their ABI names")
Local picoProducerOptions:TCompilerOptions = TestOptions()
picoProducerOptions.targetPlatform = "pico"
picoProducerOptions.targetArchitecture = "arm"
picoProducerOptions.threaded = False
picoProducerOptions.implicitRuntime = False
picoProducerOptions.sourceModuleName = "Pico.Types"
picoProducerOptions.RefreshConditionalSymbols()
Local picoTypeProducer:TCompilerResult = TBlitzMaxCompiler.Compile("types.bmx", "SuperStrict~nPublic~nGlobal producerName:String=~qproducer~q~nType TImportedReading~nField seed:Int~nMethod New(initial:Int)~nseed=initial~nEnd Method~nMethod Read:Int(value:Int)~nReturn seed+value~nEnd Method~nEnd Type", noRuntimeResolver, picoProducerOptions)
Local picoTypeProducerDiagnostics:TCompilerDiagnostic[]
Local picoTypeProducerC:String = TBlitzMaxCompiler.EmitC(picoTypeProducer, picoTypeProducerDiagnostics)
Local picoTypeProducerInitializerC:String = picoTypeProducerC[picoTypeProducerC.Find("int __bb_pico_types_types(void)")..]
Check(picoTypeProducer.Succeeded() And picoTypeProducerDiagnostics.length = 0 And Contains(picoTypeProducerC, ".abi_name = ~qpico_types_TImportedReading~q") And Contains(picoTypeProducerC, "int __bb_pico_types_types(void)") And Not Contains(picoTypeProducerC, "int main(void)") And Contains(picoTypeProducerC, "_pico_types_TImportedReading_New__Bint_ObjectNew") And Contains(picoTypeProducerInitializerC, "bmx_embedded_root_frame_enter(&bmx_embedded_global_root_frame") And Not Contains(picoTypeProducerInitializerC, "bmx_embedded_root_frame_enter(&bmx_embedded_root_frame"), "a Pico module unit exports construction and initialization ABIs, registers persistent roots, elides a safepoint-free temporary frame, and embeds its published canonical Type identity")
Local picoIncompleteInterface:TCompilerResult = TBlitzMaxCompiler.Compile("pico-incomplete-interface.bmx", "SuperStrict~nImport pico.incompletecontracts~nType TIncomplete Implements IIncomplete~nMethod Read:Int()~nReturn 1~nEnd Method~nEnd Type", noRuntimeResolver, noRuntimeOptions)
Check(Not picoIncompleteInterface.Succeeded() And HasCompilerDiagnostic(picoIncompleteInterface, "BMXC1166"), "Pico rejects an imported Interface that has no published descriptor ABI name")
Local picoClosure:TCompilerResult = TBlitzMaxCompiler.Compile("pico-closure.bmx", "SuperStrict~nFunction Increment:Int(value:Int)~nReturn value+1~nEnd Function~nType TCallableHolder~nField callback:Int(value:Int)~nEnd Type~nFunction MakeCounter:Closure<Int()>(initial:Int)~nLocal count:Int=initial~nReturn Function()~ncount:+1~nReturn count~nEnd Function~nEnd Function~nLocal thin:Int(value:Int)=Increment~nLocal missing:Int(value:Int)~nLocal holder:TCallableHolder=New TCallableHolder~nholder.callback=thin~nLocal direct:Int=holder.callback(41)~nLocal empty:Int=Not missing~nLocal counter:Closure<Int()>=MakeCounter(40)~nLocal answer:Int=counter()", noRuntimeResolver, noRuntimeOptions)
Local picoClosureDiagnostics:TCompilerDiagnostic[]
Local picoClosureC:String = TBlitzMaxCompiler.EmitC(picoClosure, picoClosureDiagnostics)
Check(picoClosure.Succeeded(), "Pico accepts thin function values and managed Closure values: " + CompilerDiagnosticSummary(picoClosure))
Check(picoClosureDiagnostics.length = 0, "Pico emits thin function values and managed Closure values without backend diagnostics: " + EmissionDiagnosticSummary(picoClosureDiagnostics))
Check(Contains(picoClosureC, "BMXEmbeddedClosure *") And Contains(picoClosureC, "bmx_embedded_closure_allocate(") And Contains(picoClosureC, "bmx_embedded_closure_assert(") And Contains(picoClosureC, "BMX_EMBEDDED_ROOT_OBJECT"), "Pico emits precisely rooted managed Closure values and captured environments")
Check(Contains(picoClosureC, "(*bmx_field_") And Contains(picoClosureC, "= 0;"), "Pico emits thin function values in locals and Object fields with a null default")
Local picoExceptionSource:String = "SuperStrict~nInterface IProblem~nMethod Code:Int()~nEnd Interface~nType TProblem Implements IProblem~nField value:Int~nMethod Code:Int()~nReturn value~nEnd Method~nEnd Type~nFunction Raise()~nThrow New TProblem~nEnd Function~nFunction Read:Int()~nTry~nTry~nRaise()~nCatch typed:TProblem~nThrow typed~nEnd Try~nCatch contract:IProblem~nReturn contract.Code()~nFinally~nLocal marker:Int=1~nEnd Try~nEnd Function"
Local picoException:TCompilerResult = TBlitzMaxCompiler.Compile("pico-exception.bmx", picoExceptionSource, noRuntimeResolver, noRuntimeOptions)
Local picoExceptionDiagnostics:TCompilerDiagnostic[]
Local picoExceptionC:String = TBlitzMaxCompiler.EmitC(picoException, picoExceptionDiagnostics)
Check(picoException.Succeeded() And picoExceptionDiagnostics.length = 0, "Pico accepts Object-backed Throw, typed Catch, rethrow and Finally control flow: " + CompilerDiagnosticSummary(picoException))
Check(Contains(picoExceptionC, "BMXEmbeddedExceptionFrame") And Contains(picoExceptionC, "switch (setjmp(") And Contains(picoExceptionC, "volatile int32_t bmx_embedded_try") And Contains(picoExceptionC, "bmx_embedded_exception_enter(") And Contains(picoExceptionC, "bmx_embedded_exception_catch(") And Contains(picoExceptionC, "bmx_embedded_exception_throw(") And Contains(picoExceptionC, "bmx_embedded_exception_leave("), "Pico exceptions lower through the embedded setjmp frame ABI with preserved post-longjmp state")
Check(Contains(picoExceptionC, "bmx_embedded_object_cast(") And Contains(picoExceptionC, "bmx_embedded_interface_cast(") And Contains(picoExceptionC, "BMX_EMBEDDED_ROOT_EXCEPTION") And Not Contains(picoExceptionC, "bbEx"), "Pico typed catches use local descriptors and a precisely rooted tagged pending exception without desktop exception calls")
Local picoStringException:TCompilerResult = TBlitzMaxCompiler.Compile("pico-string-exception.bmx", "SuperStrict~nTry~nThrow ~qproblem~q~nCatch message:String~nEnd Try", noRuntimeResolver, noRuntimeOptions)
Local picoStringExceptionDiagnostics:TCompilerDiagnostic[]
Local picoStringExceptionC:String = TBlitzMaxCompiler.EmitC(picoStringException, picoStringExceptionDiagnostics)
Check(picoStringException.Succeeded() And picoStringExceptionDiagnostics.length = 0 And Contains(picoStringExceptionC, "bmx_embedded_exception_string(") And Contains(picoStringExceptionC, ".kind == BMX_EMBEDDED_EXCEPTION_STRING"), "Pico String Throw and Catch use the tagged exception carrier without changing the compact String layout")
Local picoArrayException:TCompilerResult = TBlitzMaxCompiler.Compile("pico-array-exception.bmx", "SuperStrict~nLocal values:Int[]=New Int[2]~nTry~nThrow values~nCatch caught:Int[]~nLocal length:Int=caught.length~nEnd Try", noRuntimeResolver, noRuntimeOptions)
Local picoArrayExceptionDiagnostics:TCompilerDiagnostic[]
Local picoArrayExceptionC:String = TBlitzMaxCompiler.EmitC(picoArrayException, picoArrayExceptionDiagnostics)
Check(picoArrayException.Succeeded() And picoArrayExceptionDiagnostics.length = 0 And Contains(picoArrayExceptionC, "bmx_embedded_exception_array(") And Contains(picoArrayExceptionC, ".kind == BMX_EMBEDDED_EXCEPTION_ARRAY"), "Pico Array Throw and Catch use the tagged exception carrier without changing the compact Array layout")
Local picoUsingSource:String = "SuperStrict~nInterface ICloseable~nMethod Close()~nEnd Interface~nType TPicoResource Implements ICloseable~nField closed:Int~nField fail:Int~nMethod Close()~nclosed:+1~nIf fail Then Throw ~qclose~q~nEnd Method~nEnd Type~nLocal first:TPicoResource=New TPicoResource~nLocal second:TPicoResource=New TPicoResource~nsecond.fail=True~nTry~nUsing~nLocal outer:TPicoResource=first~nLocal inner:TPicoResource=second~nDo~nThrow ~qbody~q~nEnd Using~nCatch message:String~nEnd Try"
Local picoUsing:TCompilerResult = TBlitzMaxCompiler.Compile("pico-using.bmx", picoUsingSource, noRuntimeResolver, noRuntimeOptions)
Local picoUsingDiagnostics:TCompilerDiagnostic[]
Local picoUsingC:String = TBlitzMaxCompiler.EmitC(picoUsing, picoUsingDiagnostics)
Check(picoUsing.Succeeded() And picoUsingDiagnostics.length = 0, "Pico accepts Using with ICloseable resources and exception cleanup: " + CompilerDiagnosticSummary(picoUsing))
Check(Contains(picoUsingC, "ICloseable") And Contains(picoUsingC, "bmx_embedded_interface_methods(") And Occurrences(picoUsingC, "switch (setjmp(") >= 3 And Contains(picoUsingC, "volatile int32_t bmx_embedded_") And Occurrences(picoUsingC, "(void)bmx_embedded_exception_catch();") = 2 And Contains(picoUsingC, "_failed) bmx_embedded_exception_throw("), "Pico Using protects its body, preserves post-longjmp state, closes resources in guarded frames, suppresses Close exceptions, and rethrows the original tagged exception")
Local picoUsingReturn:TCompilerResult = TBlitzMaxCompiler.Compile("pico-using-return.bmx", "SuperStrict~nInterface ICloseable~nMethod Close()~nEnd Interface~nType TPicoReturnResource Implements ICloseable~nMethod Close()~nEnd Method~nEnd Type~nFunction Read:Int()~nUsing~nLocal resource:TPicoReturnResource=New TPicoReturnResource~nDo~nReturn 41~nEnd Using~nEnd Function", noRuntimeResolver, noRuntimeOptions)
Local picoUsingReturnDiagnostics:TCompilerDiagnostic[]
Local picoUsingReturnC:String = TBlitzMaxCompiler.EmitC(picoUsingReturn, picoUsingReturnDiagnostics)
Check(picoUsingReturn.Succeeded() And picoUsingReturnDiagnostics.length = 0 And Contains(picoUsingReturnC, "bmx_cleanup_return_0 = 41;") And AppearsBefore(picoUsingReturnC, "bmx_embedded_exception_leave();", "return bmx_cleanup_return_0;") And AppearsBefore(picoUsingReturnC, "bmx_embedded_interface_methods(", "return bmx_cleanup_return_0;"), "Pico Return across Using preserves its value, leaves the body handler, closes the resource, and then returns")
Local picoObjectValues:TCompilerResult = TBlitzMaxCompiler.Compile("pico-object-values.bmx", "SuperStrict~nType TKey~nField value:Int~nMethod ToString:String() Override~nReturn ~qkey~q~nEnd Method~nMethod Compare:Int(other:Object) Override~nReturn value~nEnd Method~nMethod HashCode:UInt() Override~nReturn UInt(value * 17)~nEnd Method~nMethod Equals:Int(other:Object) Override~nReturn Self = other~nEnd Method~nEnd Type~nType TPlain~nEnd Type~nLocal key:TKey = New TKey~nkey.value = 3~nLocal text:String = key.ToString()~nLocal ordering:Int = key.Compare(key)~nLocal hash:UInt = key.HashCode()~nLocal equal:Int = key.Equals(key)~nLocal base:Object = key~nLocal baseText:String = base.ToString()~nLocal castText:String = String(base)~nLocal baseOrdering:Int = base.Compare(key)~nLocal baseHash:UInt = base.HashCode()~nLocal baseEqual:Int = base.Equals(key)~nLocal plain:TPlain = New TPlain~nLocal plainText:String = plain.ToString()~nLocal defaultHash:UInt = plain.HashCode()~nLocal defaultEqual:Int = plain.Equals(plain)", noRuntimeResolver, noRuntimeOptions)
Local picoObjectValueDiagnostics:TCompilerDiagnostic[]
Local picoObjectValueC:String = TBlitzMaxCompiler.EmitC(picoObjectValues, picoObjectValueDiagnostics)
Check(picoObjectValues.Succeeded() And picoObjectValueDiagnostics.length = 0 And Contains(picoObjectValueC, ".to_string = bmx_embedded_to_string_") And Contains(picoObjectValueC, ".compare = bmx_embedded_compare_") And Contains(picoObjectValueC, ".hash_code = bmx_embedded_hash_code_") And Contains(picoObjectValueC, ".equals = bmx_embedded_equals_") And Occurrences(picoObjectValueC, "bmx_embedded_object_to_string(") >= 3 And Occurrences(picoObjectValueC, "bmx_embedded_object_compare(") >= 2 And Occurrences(picoObjectValueC, "bmx_embedded_object_hash_code(") >= 3 And Occurrences(picoObjectValueC, "bmx_embedded_object_equals(") >= 3, "Pico Object value operations use compact descriptor hooks with runtime defaults through concrete and Object-typed receivers")
Local picoFinalizer:TCompilerResult = TBlitzMaxCompiler.Compile("pico-finalizer.bmx", "SuperStrict~nType TResource~nField value:Int~nMethod Delete()~nvalue = 0~nEnd Method~nEnd Type~nLocal resource:TResource = New TResource~nresource = Null", noRuntimeResolver, noRuntimeOptions)
Local picoFinalizerDiagnostics:TCompilerDiagnostic[]
Local picoFinalizerC:String = TBlitzMaxCompiler.EmitC(picoFinalizer, picoFinalizerDiagnostics)
Check(picoFinalizer.Succeeded() And picoFinalizerDiagnostics.length = 0 And Contains(picoFinalizerC, "BMX_EMBEDDED_TYPE_FLAG_HAS_FINALIZER") And Contains(picoFinalizerC, ".finalizer = bmx_embedded_finalize_") And Contains(picoFinalizerC, "static void bmx_embedded_finalize_") And Contains(picoFinalizerC, "bmx_fn0_Delete(") And Not Contains(picoFinalizerC, "bbObjectDtor"), "Pico Object Delete methods emit descriptor-backed finalizer thunks without desktop runtime destruction")
Local picoReferenceField:TCompilerResult = TBlitzMaxCompiler.Compile("pico-reference-field.bmx", "SuperStrict~nType TNode~nField nextNode:TNode~nEnd Type~nLocal tail:TNode = New TNode~nLocal head:TNode = New TNode~nhead.nextNode = tail", noRuntimeResolver, noRuntimeOptions)
Local picoReferenceFieldDiagnostics:TCompilerDiagnostic[]
Local picoReferenceFieldC:String = TBlitzMaxCompiler.EmitC(picoReferenceField, picoReferenceFieldDiagnostics)
Check(picoReferenceField.Succeeded() And picoReferenceFieldDiagnostics.length = 0 And Contains(picoReferenceFieldC, "static const uint32_t bmx_embedded_type_") And Contains(picoReferenceFieldC, "(uint32_t)offsetof(struct ") And Contains(picoReferenceFieldC, ".reference_count = 1"), "Pico embedded Object fields emit precise 32-bit descriptor offsets")
Local picoBaseObject:TCompilerResult = TBlitzMaxCompiler.Compile("pico-base-object.bmx", "SuperStrict~nLocal value:Object", noRuntimeResolver, noRuntimeOptions)
Local picoBaseObjectDiagnostics:TCompilerDiagnostic[]
Local picoBaseObjectC:String = TBlitzMaxCompiler.EmitC(picoBaseObject, picoBaseObjectDiagnostics)
Check(picoBaseObject.Succeeded() And picoBaseObjectDiagnostics.length = 0 And Contains(picoBaseObjectC, "BMXEmbeddedObject *") And Contains(picoBaseObjectC, "&bmx_embedded_null_object") And Not Contains(picoBaseObjectC, "bbNullObject"), "Pico base Object values use the embedded object ABI and null sentinel")
Local picoManagedStringField:TCompilerResult = TBlitzMaxCompiler.Compile("pico-managed-string-field.bmx", "SuperStrict~nType TNamed~nField name:String~nEnd Type~nLocal value:TNamed = New TNamed", noRuntimeResolver, noRuntimeOptions)
Local picoManagedStringFieldDiagnostics:TCompilerDiagnostic[]
Local picoManagedStringFieldC:String = TBlitzMaxCompiler.EmitC(picoManagedStringField, picoManagedStringFieldDiagnostics)
Check(picoManagedStringField.Succeeded() And picoManagedStringFieldDiagnostics.length = 0 And Contains(picoManagedStringFieldC, "const BMXEmbeddedString *") And Contains(picoManagedStringFieldC, "_strings[]") And Contains(picoManagedStringFieldC, ".string_count = 1"), "Pico embedded Objects publish precise managed String field offsets")
Local picoManagedArrayField:TCompilerResult = TBlitzMaxCompiler.Compile("pico-managed-array-field.bmx", "SuperStrict~nType TItem~nField value:Int~nEnd Type~nType TBucket~nField names:String[]~nField items:TItem[]~nEnd Type~nLocal value:TBucket = New TBucket", noRuntimeResolver, noRuntimeOptions)
Local picoManagedArrayFieldDiagnostics:TCompilerDiagnostic[]
Local picoManagedArrayFieldC:String = TBlitzMaxCompiler.EmitC(picoManagedArrayField, picoManagedArrayFieldDiagnostics)
Check(picoManagedArrayField.Succeeded() And picoManagedArrayFieldDiagnostics.length = 0 And Contains(picoManagedArrayFieldC, "_arrays[]") And Contains(picoManagedArrayFieldC, ".array_count = 2"), "Pico Object descriptors emit precise managed Array field offsets")
...
Local picoPublicStructOptions:TCompilerOptions = TestOptions()
picoPublicStructOptions.targetPlatform = "pico"
picoPublicStructOptions.targetArchitecture = "arm"
picoPublicStructOptions.implicitRuntime = False
picoPublicStructOptions.RefreshConditionalSymbols()
Local picoPublicStructConsumer:TCompilerResult = TBlitzMaxCompiler.Compile("pico-public-struct-consumer.bmx", "SuperStrict~nImport acme.values~nLocal value:SValue=New SValue(19)~nvalue.Move(1)~nLocal copy:SValue=value~nLocal values:SValue[]=New SValue[2]~nvalues[0]=copy~nLocal total:Int=value.Sum()+values[0].first", resolver, picoPublicStructOptions)
Local picoPublicStructConsumerDiagnostics:TCompilerDiagnostic[]
Local picoPublicStructConsumerC:String = TBlitzMaxCompiler.EmitC(picoPublicStructConsumer, picoPublicStructConsumerDiagnostics)
Check(picoPublicStructConsumer.Succeeded() And picoPublicStructConsumerDiagnostics.length = 0 And Contains(picoPublicStructConsumerC, "struct acme_values_SValue {") And Contains(picoPublicStructConsumerC, "extern void _acme_values_SValue_Move__Bint(struct acme_values_SValue *") And Contains(picoPublicStructConsumerC, "extern int32_t _acme_values_SValue_Sum(struct acme_values_SValue *") And Contains(picoPublicStructConsumerC, "extern void bbStructElementInit_acme_values_SValue(void *bmx_value);") And Contains(picoPublicStructConsumerC, "BMX_EMBEDDED_ARRAY_ELEMENT_VALUE, bbStructElementInit_acme_values_SValue, 0") And Contains(picoPublicStructConsumerC, "_acme_values_SValue_Sum((&bmx_v0_value))"), "Pico consumers reconstruct imported plain-data Struct layouts, direct method implementation ABI and producer-owned Array initialization")
...
Local picoReturnedMethodCallablePublicConsumer:TCompilerResult = TBlitzMaxCompiler.Compile("pico-returned-method-callable-public-consumer.bmx", returnedMethodCallablePublicConsumerSource, resolver, picoPublicStructOptions)
Local picoReturnedMethodCallablePublicConsumerDiagnostics:TCompilerDiagnostic[]
Local picoReturnedMethodCallablePublicConsumerC:String = TBlitzMaxCompiler.EmitC(picoReturnedMethodCallablePublicConsumer, picoReturnedMethodCallablePublicConsumerDiagnostics)
Check(picoReturnedMethodCallablePublicConsumer.Succeeded() And picoReturnedMethodCallablePublicConsumerDiagnostics.length = 0 And Contains(picoReturnedMethodCallablePublicConsumerC, "int32_t (*(*)(struct acme_returnedmethodcallbacks_TReturnedBase_obj *, int32_t))(int32_t *)") And Contains(picoReturnedMethodCallablePublicConsumerC, "int32_t (*(*)(BMXEmbeddedObject *, int32_t))(int32_t *)") And Contains(picoReturnedMethodCallablePublicConsumerC, "bmx_embedded_type_methods((void *)") And Contains(picoReturnedMethodCallablePublicConsumerC, "bmx_embedded_interface_methods((void *)"), "Pico separate compilation preserves callable-return signatures through imported compact virtual and Interface method tables: " + CompilerDiagnosticSummary(picoReturnedMethodCallablePublicConsumer) + "; emission: " + EmissionDiagnosticSummary(picoReturnedMethodCallablePublicConsumerDiagnostics))
Local picoMessagePublic:TCompilerResult = TBlitzMaxCompiler.Compile("/sdk/mod/acme.mod/messages.mod/messages.bmx", "SuperStrict~nModule acme.messages~nType TMessageReceiver~nMethod SendMessage:Object(message:Object,source:Object) Override~nReturn message~nEnd Method~nEnd Type~nFunction CreateMessageReceiver:TMessageReceiver()~nReturn New TMessageReceiver~nEnd Function", resolver, picoPublicStructOptions)
Local picoMessagePublicInterfaceDiagnostics:TCompilerDiagnostic[]
Local picoMessagePublicInterface:String = TBlitzMaxCompiler.EmitInterface(picoMessagePublic, picoMessagePublicInterfaceDiagnostics)
Local picoMessagePublicDiagnostics:TCompilerDiagnostic[]
Local picoMessagePublicC:String = TBlitzMaxCompiler.EmitC(picoMessagePublic, picoMessagePublicDiagnostics)
Check(picoMessagePublic.Succeeded() And picoMessagePublicInterfaceDiagnostics.length = 0 And picoMessagePublicDiagnostics.length = 0 And Contains(picoMessagePublicC, ".send_message = bmx_embedded_send_message_") And Contains(picoMessagePublicC, "return _acme_messages_TMessageReceiver_SendMessage"), "Pico module producers publish Object.SendMessage in their compact type descriptor")
resolver.AddInterface("acme.messages", "sdk/acme.messages.i", picoMessagePublicInterface)
Local picoMessagePublicConsumer:TCompilerResult = TBlitzMaxCompiler.Compile("pico-message-public-consumer.bmx", "SuperStrict~nImport acme.messages~nLocal receiver:Object=CreateMessageReceiver()~nLocal payload:Object=CreateMessageReceiver()~nLocal result:Object=receiver.SendMessage(payload,Null)", resolver, picoPublicStructOptions)
Local picoMessagePublicConsumerDiagnostics:TCompilerDiagnostic[]
Local picoMessagePublicConsumerC:String = TBlitzMaxCompiler.EmitC(picoMessagePublicConsumer, picoMessagePublicConsumerDiagnostics)
Check(picoMessagePublicConsumer.Succeeded() And picoMessagePublicConsumerDiagnostics.length = 0 And Contains(picoMessagePublicConsumerC, "bmx_embedded_object_send_message((void *)"), "Pico source-free consumers dispatch imported Object.SendMessage through the producer-owned compact hook")
...
Local picoManagedPublicStruct:TCompilerResult = TBlitzMaxCompiler.Compile("/sdk/mod/acme.mod/managedvalues.mod/managedvalues.bmx", managedPublicStructSource, resolver, picoPublicStructOptions)
Local picoManagedPublicStructDiagnostics:TCompilerDiagnostic[]
Local picoManagedPublicStructC:String = TBlitzMaxCompiler.EmitC(picoManagedPublicStruct, picoManagedPublicStructDiagnostics)
Check(picoManagedPublicStruct.Succeeded() And picoManagedPublicStructDiagnostics.length = 0 And Contains(picoManagedPublicStructC, "void bbStructElementInit_acme_managedvalues_SManagedValue(void *bmx_value)") And Not Contains(picoManagedPublicStructC, "BBArray *bbArrayNew1DStruct_acme_managedvalues_SManagedValue"), "a Pico producer publishes the target-neutral Struct element-initializer ABI without desktop Array wrappers")
Local picoManagedPublicStructConsumer:TCompilerResult = TBlitzMaxCompiler.Compile("pico-managed-public-struct-consumer.bmx", "SuperStrict~nImport acme.managedvalues~nStruct SEnvelope~nField StaticArray values:SManagedValue[2]~nEnd Struct~nType THolder~nField value:SManagedValue~nField envelope:SEnvelope~nField StaticArray fixed:SManagedValue[2]~nEnd Type~nLocal values:SManagedValue[]=New SManagedValue[2]~nvalues[0].text=~qkept~q~nvalues[0].items=New Int[1]~nLocal StaticArray roots:SManagedValue[2]~nroots[0]=values[0]~nLocal holder:THolder=New THolder~nholder.value=roots[0]~nholder.envelope.values[0]=holder.value~nholder.fixed[0]=holder.value", resolver, picoPublicStructOptions)
Local picoManagedPublicStructConsumerDiagnostics:TCompilerDiagnostic[]
Local picoManagedPublicStructConsumerC:String = TBlitzMaxCompiler.EmitC(picoManagedPublicStructConsumer, picoManagedPublicStructConsumerDiagnostics)
Check(picoManagedPublicStructConsumer.Succeeded() And picoManagedPublicStructConsumerDiagnostics.length = 0 And Contains(picoManagedPublicStructConsumerC, "static const BMXEmbeddedValueDescriptor bmx_embedded_imported_value_") And Contains(picoManagedPublicStructConsumerC, "BMX_EMBEDDED_VALUE_STRING") And Contains(picoManagedPublicStructConsumerC, "BMX_EMBEDDED_VALUE_ARRAY") And Contains(picoManagedPublicStructConsumerC, "BMX_EMBEDDED_VALUE_OBJECT") And Contains(picoManagedPublicStructConsumerC, "BMX_EMBEDDED_ARRAY_ELEMENT_VALUE, bbStructElementInit_acme_managedvalues_SManagedValue, &bmx_embedded_imported_value_") And Contains(picoManagedPublicStructConsumerC, "BMX_EMBEDDED_VALUE_STRUCT, &bmx_embedded_imported_value_") And Contains(picoManagedPublicStructConsumerC, "(uint32_t)sizeof(struct acme_managedvalues_SManagedValue), 2, BMX_EMBEDDED_VALUE_STRUCT") And Contains(picoManagedPublicStructConsumerC, "BMX_EMBEDDED_ROOT_STRUCT, &bmx_embedded_imported_value_") And Contains(picoManagedPublicStructConsumerC, "_roots[0]") And Contains(picoManagedPublicStructConsumerC, "_roots[1]"), "Pico imported managed Struct fields, dynamic Arrays and StaticArrays share recursive 32-bit descriptors across roots, nested Structs and containing Objects")
