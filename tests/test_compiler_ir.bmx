...
Local supportedTargets:String[] = ["macos/x86", "macos/x64", "macos/ppc", "macos/arm64", "win32/x86", "win32/x64", "win32/armv7", "win32/arm64", "linux/x86", "linux/x64", "linux/arm", "linux/arm64", "linux/riscv32", "linux/riscv64", "haiku/x86", "haiku/x64", "haiku/arm64", "pico/arm"]
...
Next
Local rejectedTargets:String[] = ["macos/riscv64", "win32/arm", "linux/ppc", "windows/x64", "linux/unknown", "pico/arm64", "pico/riscv32"]
...
Local picoGenericObjectHooksSource:String = "SuperStrict~nGlobal baseCalls:UInt~nGlobal derivedCalls:UInt~nType THookBase<T>~nMethod Compare:Int(other:Object)~nbaseCalls:+1~nReturn -10~nEnd Method~nMethod HashCode:UInt()~nbaseCalls:+1~nReturn 100~nEnd Method~nMethod Equals:Int(other:Object)~nbaseCalls:+1~nReturn True~nEnd Method~nEnd Type~nType THookDerived<T> Extends THookBase<T>~nMethod Compare:Int(other:Object) Override~nderivedCalls:+1~nReturn -20~nEnd Method~nMethod HashCode:UInt() Override~nderivedCalls:+1~nReturn 200~nEnd Method~nMethod Equals:Int(other:Object) Override~nderivedCalls:+1~nReturn False~nEnd Method~nEnd Type~nType THookInherited<T> Extends THookBase<T>~nEnd Type~nGlobal base:Object=New THookBase<Int>~nGlobal derived:Object=New THookDerived<Int>~nGlobal inherited:Object=New THookInherited<Int>~nGlobal baseCompare:Int=base.Compare(derived)~nGlobal baseHash:UInt=base.HashCode()~nGlobal baseEquals:Int=base.Equals(derived)~nGlobal derivedCompare:Int=derived.Compare(base)~nGlobal derivedHash:UInt=derived.HashCode()~nGlobal derivedEquals:Int=derived.Equals(base)~nGlobal inheritedCompare:Int=inherited.Compare(base)~nGlobal inheritedHash:UInt=inherited.HashCode()~nGlobal inheritedEquals:Int=inherited.Equals(base)"
...
Check(Occurrences(picoGenericObjectHookUnits, ".compare = bmx_gen_") = 3 And Occurrences(picoGenericObjectHookUnits, ".hash_code = bmx_gen_") = 3 And Occurrences(picoGenericObjectHookUnits, ".equals = bmx_gen_") = 3 And Occurrences(picoGenericObjectHookUnits, "static int32_t bmx_gen_") >= 6 And Occurrences(picoGenericObjectHookUnits, "static uint32_t bmx_gen_") >= 3 And picoGenericObjectHookUnits.Contains("extern uint32_t"), "Pico generic descriptors publish signature-matched Object hooks and wrappers for inherited and overridden implementations")
...
Local picoObjectSource:String = "SuperStrict~nType TCounter~nField value:Int~nMethod New(initial:Int)~nvalue = initial~nEnd Method~nMethod Add:Int(delta:Int)~nvalue :+ delta~nReturn value~nEnd Method~nEnd Type~nLocal counter:TCounter = New TCounter(4)~nLocal result:Int = counter.Add(3)"
...
Check(picoObject.Succeeded() And picoObjectDiagnostics.length = 0 And Contains(picoObjectC, "BMXEmbeddedObject object;") And Contains(picoObjectC, "static const BMXEmbeddedTypeDescriptor bmx_embedded_type_") And Contains(picoObjectC, "bmx_embedded_object_allocate(&bmx_embedded_type_") And Contains(picoObjectC, "bmx_embedded_object_not_null(") And Contains(picoObjectC, "BMXEmbeddedRootFrame bmx_embedded_root_frame;") And Contains(picoObjectC, "bmx_embedded_root_frame_enter(") And Contains(picoObjectC, "bmx_embedded_root_frame_leave(") And Contains(picoObjectC, "bmx_fn1_Add("), "Pico standalone C emits described arena-backed concrete Objects with safepoint-driven root frames, cheap release receiver checks, constructors, fields and virtual method calls")
...
Check(Contains(picoExceptionC, "BMXEmbeddedExceptionFrame") And Contains(picoExceptionC, "switch (setjmp(") And Contains(picoExceptionC, "bmx_embedded_exception_enter(") And Contains(picoExceptionC, "bmx_embedded_exception_catch(") And Contains(picoExceptionC, "bmx_embedded_exception_throw(") And Contains(picoExceptionC, "bmx_embedded_exception_leave("), "Pico exceptions lower through the embedded setjmp frame ABI")
...
Check(Contains(picoUsingC, "ICloseable") And Contains(picoUsingC, "bmx_embedded_interface_methods(") And Occurrences(picoUsingC, "switch (setjmp(") >= 3 And Occurrences(picoUsingC, "(void)bmx_embedded_exception_catch();") = 2 And Contains(picoUsingC, "_failed) bmx_embedded_exception_throw("), "Pico Using protects its body, closes resources in guarded frames, suppresses Close exceptions, and rethrows the original tagged exception")
...
Local picoObjectValues:TCompilerResult = TBlitzMaxCompiler.Compile("pico-object-values.bmx", "SuperStrict~nType TKey~nField value:Int~nMethod Compare:Int(other:Object) Override~nReturn value~nEnd Method~nMethod HashCode:UInt() Override~nReturn UInt(value * 17)~nEnd Method~nMethod Equals:Int(other:Object) Override~nReturn Self = other~nEnd Method~nEnd Type~nType TPlain~nEnd Type~nLocal key:TKey = New TKey~nkey.value = 3~nLocal ordering:Int = key.Compare(key)~nLocal hash:UInt = key.HashCode()~nLocal equal:Int = key.Equals(key)~nLocal base:Object = key~nLocal baseOrdering:Int = base.Compare(key)~nLocal baseHash:UInt = base.HashCode()~nLocal baseEqual:Int = base.Equals(key)~nLocal plain:TPlain = New TPlain~nLocal defaultHash:UInt = plain.HashCode()~nLocal defaultEqual:Int = plain.Equals(plain)", noRuntimeResolver, noRuntimeOptions)
