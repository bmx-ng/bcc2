...
Check(HasConditionalSymbol(picoTargetSymbols, "pico") And HasConditionalSymbol(picoTargetSymbols, "arm") And HasConditionalSymbol(picoTargetSymbols, "picoarm") And HasConditionalSymbol(picoTargetSymbols, "ptr32") And HasConditionalSymbol(picoTargetSymbols, "longint4") And HasConditionalSymbol(picoTargetSymbols, "littleendian") And Not HasConditionalSymbol(picoTargetSymbols, "linux") And Not HasConditionalSymbol(picoTargetSymbols, "threaded"), "Pico/arm exposes a 32-bit little-endian bare-metal target without desktop or threading aliases")
...
Check(noRuntimeScalar.Succeeded() And noRuntimeScalarDiagnostics.length = 0 And Contains(noRuntimeScalarC, "int main(void)") And Contains(noRuntimeScalarC, "bmx_pico_modules_init();") And Not Contains(noRuntimeScalarC, "bmx_pico_root_frame_enter(&bmx_pico_root_frame") And Contains(noRuntimeScalarC, "value *= 2;"), "runtime-free Pico scalar compilation retains the semantic-model to typed-IR to standalone-C pipeline and elides a root frame when the typed body has no GC safepoint")
...
Check(picoIncbin.Succeeded() And picoIncbinDiagnostics.length = 0 And Contains(picoIncbinC, "extern const unsigned char *_ib_bb_main_1_data;") And Contains(picoIncbinC, "extern const unsigned int _ib_bb_main_1_size;") And Contains(picoIncbinC, "bbIncbinAdd(&bmx_pico_string_str0, &_ib_bb_main_1_data, (int32_t)_ib_bb_main_1_size);") And AppearsBefore(picoIncbinC, "bmx_pico_modules_init();", "bbIncbinAdd("), "Pico standalone C declares packaged Incbin symbols and registers flash resources after imported modules initialize")
...
Check(picoMilliSecs.Succeeded() And picoMilliSecsDiagnostics.length = 0 And Contains(picoMilliSecsC, "bmx_pico_millisecs(") And Not Contains(picoMilliSecsC, "bbMilliSecs("), "Pico standard MilliSecs lowers directly to the embedded monotonic clock")
...
Check(picoGenericStructUnit.content.Contains("const BMXPicoString *") And picoGenericStructUnit.content.Contains("&bmx_pico_empty_string") And picoGenericStructUnit.content.Contains("bbStructElementInit_"), "Pico generic Struct units use the embedded managed-value ABI and publish element initialization")
...
Check(picoGenericStructUnit.content.Contains("const BMXPicoValueDescriptor bmx_gen_") And picoGenericStructUnit.content.Contains("_pico_value_descriptor =") And picoGenericStructUnit.content.Contains("BMX_PICO_VALUE_STRING") And picoGenericStructUnit.content.Contains("BMX_PICO_ROOT_STRUCT") And picoGenericStructUnit.content.Contains("BMX_PICO_ROOT_STRING") And picoGenericStructUnit.content.Contains("bmx_pico_root_frame_enter(&bmx_pico_generic_root_frame") And picoGenericStructUnit.content.Contains("bmx_pico_root_frame_leave(&bmx_pico_generic_root_frame)") And picoGenericStructUnit.content.Contains("bmx_pico_string_concat("), "Pico generic Struct units publish precise descriptors and root Self, managed parameters, and managed locals around allocating String operations")
Check(picoGenericStructApplication And picoGenericStructApplication.content.Contains("BMX_PICO_VALUE_STRING") And picoGenericStructApplication.content.Contains("BMX_PICO_ROOT_STRUCT") And picoGenericStructApplication.content.Contains("bmx_pico_array_new_1d") And picoGenericStructApplication.content.Contains("bbStructElementInit_"), "Pico consumers emit precise descriptors and root slots for managed generic Struct values and Arrays")
...
Check(picoNestedGenericStructUnits.Contains("extern const BMXPicoValueDescriptor bmx_gen_") And picoNestedGenericStructUnits.Contains("_pico_value_descriptor;") And picoNestedGenericStructUnits.Contains("bbStructElementInit_") And picoNestedGenericStructUnits.Contains("bmx_pico_array_new_1d") And picoNestedGenericStructUnits.Contains("bmx_pico_array_slice"), "Pico generic specialization units share the published Struct descriptor and initializer across allocation and slicing")
Check(picoNestedGenericStructUnits.Contains("BMX_PICO_VALUE_STRUCT") And picoNestedGenericStructUnits.Contains(".value_field_count = 1") And picoNestedGenericStructUnits.Contains("BMX_PICO_ROOT_STRUCT"), "Pico generic Type descriptors and root frames trace managed references nested inside generic Struct values")
...
Check(picoGenericTypeUnit.content.Contains("BMXPicoObject object;") And picoGenericTypeUnit.content.Contains("const BMXPicoTypeDescriptor") And picoGenericTypeUnit.content.Contains("bmx_pico_object_allocate(") And picoGenericTypeUnit.content.Contains("BMX_PICO_ROOT_OBJECT"), "Pico managed generic Type units publish the embedded object descriptor and root allocation during construction")
...
Check(picoGenericOrdinaryAbiUnit.content.Contains("#include ~q../../application.h~q") And picoGenericOrdinaryAbiUnit.content.Contains("bmx_pico_type_methods((void *)") And picoGenericOrdinaryAbiUnit.content.Contains(", 0, 1)") And Not picoGenericOrdinaryAbiUnit.content.Contains("->clas->vfns"), "Pico generic units import the shared Struct layout, emit local helpers, and dispatch ordinary Types through embedded method tables")
Check(picoGenericOrdinaryAbiUnit.content.Contains("int8_t *") And picoGenericOrdinaryAbiUnit.content.Contains("BMXPicoArray *"), "Pico generic Type fields retain pointer and pointer-Array storage")
...
Check(picoGenericContainerUnits.Contains("BMXPicoInterfaceDescriptor") And picoGenericContainerUnits.Contains("BMXPicoInterfaceEntry") And picoGenericContainerUnits.Contains("bmx_pico_array_new_1d") And picoGenericContainerUnits.Contains("bmx_pico_array_element") And picoGenericContainerUnits.Contains("bmx_pico_array_slice"), "Pico generic containers publish interface metadata and lower managed Array allocation, access and slicing through the embedded ABI")
Check(picoGenericContainerApplication And picoGenericContainerApplication.content.Contains("bmx_pico_interface_methods("), "Pico application consumers dispatch imported canonical generic Interfaces through embedded descriptors")
...
Check(picoNestedGenericContainerUnits.Contains("BMXPicoInterfaceDescriptor"), "Pico nested generic units declare referenced Interface descriptors")
...
Check(picoNestedGenericContainerUnits.Contains("bmx_pico_object_cast(") And Not picoNestedGenericContainerUnits.Contains("bbObjectDowncast("), "Pico nested generic units lower explicit generic object casts through the embedded ABI")
...
Check(picoGenericInheritanceUnits.Contains(".super = &bmx_gen_") And picoGenericInheritanceUnits.Contains(".string_count = 2") And picoGenericInheritanceUnits.Contains("bmx_pico_new_root_slots[3]") And picoGenericInheritanceUnits.Contains("bmx_pico_type_methods((void *)") And picoGenericInheritanceUnits.Contains("BMXPicoInterfaceEntry"), "Pico derived generic descriptors flatten precise managed-field metadata, root managed constructor arguments, link their base descriptor, and retain virtual and Interface dispatch")
...
Check(picoGenericInheritanceApplication And Occurrences(picoGenericInheritanceApplication.content, "bmx_pico_type_methods((void *)") >= 3 And picoGenericInheritanceApplication.content.Contains("bmx_pico_interface_methods((void *)"), "Pico application consumers use descriptor slots for base, derived, Self-dispatched and Interface generic calls")
...
Check(picoGenericObjectHookApplication And Occurrences(picoGenericObjectHookApplication.content, "bmx_pico_object_compare(") = 3 And Occurrences(picoGenericObjectHookApplication.content, "bmx_pico_object_hash_code(") = 3 And Occurrences(picoGenericObjectHookApplication.content, "bmx_pico_object_equals(") = 3, "Pico application consumers dispatch generic Compare, HashCode and Equals calls through descriptor hooks")
...
Check(picoString.Succeeded() And picoStringDiagnostics.length = 0 And Contains(picoStringC, "#include <blitzmax/pico_runtime.h>") And Contains(picoStringC, "static const BMXPicoString bmx_pico_string_str0") And Contains(picoStringC, "->length") And Contains(picoStringC, "bmx_pico_string_equals("), "Pico standalone C emits immutable embedded String literals, length access and comparison through the embedded runtime ABI")
...
Check(picoStringMethods.Succeeded() And picoStringMethodDiagnostics.length = 0 And Contains(picoStringMethodC, "bmx_pico_string_to_string(") And Contains(picoStringMethodC, "bmx_pico_string_compare(") And Contains(picoStringMethodC, "bmx_pico_string_equals(") And Contains(picoStringMethodC, "bmx_pico_string_hash(") And Not Contains(picoStringMethodC, "extern const BMXPicoString * bbString"), "Pico String value methods lower directly to the compact embedded runtime ABI")
...
Check(picoDynamicString.Succeeded() And picoDynamicStringDiagnostics.length = 0 And Occurrences(picoDynamicStringC, "bmx_pico_string_concat(") = 3 And Contains(picoDynamicStringC, "bmx_pico_string_slice(") And Contains(picoDynamicStringC, "bmx_pico_string_from_char(") And Contains(picoDynamicStringC, "BMX_PICO_ROOT_STRING") And Contains(picoDynamicStringC, "bmx_tmp_"), "Pico dynamic String operations lower through rooted, explicitly sequenced typed IR temporaries")
...
Check(picoEnum.Succeeded() And picoEnumDiagnostics.length = 0 And Contains(picoEnumC, "uint8_t bmx_v0_mode = 7") And Contains(picoEnumC, "BMX_PICO_ARRAY_ELEMENT_VALUE, 0") And Contains(picoEnumC, "bmx_pico_array_element("), "Pico scalar Enums retain their underlying representation, Ordinal operation, Arrays, indexing and iteration")
...
Check(picoEnumReflection.Succeeded() And picoEnumReflectionDiagnostics.length = 0 And Contains(picoEnumReflectionC, "static const BMXPicoEnumDescriptor") And Contains(picoEnumReflectionC, "bmx_pico_enum_to_string(") And Contains(picoEnumReflectionC, "bmx_pico_enum_values(") And Contains(picoEnumReflectionC, "bmx_pico_enum_try_convert(") And Contains(picoEnumReflectionC, "bmx_pico_enum_from_string("), "Pico local Enums emit compact flash descriptors for names, Values, TryConvert and case-insensitive parsing")
...
Check(picoStruct.Succeeded() And picoStructDiagnostics.length = 0 And Contains(picoStructC, "struct bmx_struct_") And Contains(picoStructC, "bmx_struct_new_") And Contains(picoStructC, "bmx_struct_array_init_") And Contains(picoStructC, "BMX_PICO_ARRAY_ELEMENT_VALUE, bmx_struct_array_init_") And Contains(picoStructC, "bmx_pico_array_element("), "Pico unmanaged Structs support construction, methods, nesting, value copy, Object fields and initialized one-dimensional Arrays")
...
Check(picoArray.Succeeded() And picoArrayDiagnostics.length = 0 And Contains(picoArrayC, "BMXPicoArray *") And Contains(picoArrayC, "bmx_pico_array_new_1d(") And Contains(picoArrayC, "bmx_pico_array_element(") And Contains(picoArrayC, "->length") And Contains(picoArrayC, "BMXPicoRootSlot bmx_pico_root_slots") And Contains(picoArrayC, "BMX_PICO_ROOT_ARRAY"), "Pico standalone C emits arena-backed scalar Arrays with typed roots even when the module declares no Object Types")
...
Check(picoArrayConcat.Succeeded() And picoArrayConcatDiagnostics.length = 0 And Occurrences(picoArrayConcatC, "bmx_pico_array_from_data(") = 4 And Occurrences(picoArrayConcatC, "bmx_pico_array_concat(") = 2, "Pico Array literals, concatenation and :+ assignment lower through the embedded managed-container ABI: " + CompilerDiagnosticSummary(picoArrayConcat) + "; emission: " + EmissionDiagnosticSummary(picoArrayConcatDiagnostics))
Check(Contains(picoArrayConcatC, "BMX_PICO_ARRAY_ELEMENT_VALUE") And Contains(picoArrayConcatC, "BMX_PICO_ARRAY_ELEMENT_STRING") And Contains(picoArrayConcatC, "BMX_PICO_ROOT_STRING") And Contains(picoArrayConcatC, "BMX_PICO_ROOT_ARRAY") And Contains(picoArrayConcatC, "bmx_tmp_"), "Pico Array literals and concatenation preserve precise roots for materialized elements and the sequenced left operand")
...
Check(Occurrences(picoBlockMemoryC, "bmx_pico_array_data(") >= 4 And Contains(picoBlockMemoryC, "brl_blitz_MemAlloc__Bsize_t__Bint") And Contains(picoBlockMemoryC, "brl_blitz_MemExtend__PBbyte__Bsize_t__Bsize_t__Bint") And Contains(picoBlockMemoryC, "brl_blitz_MemFree__PBbyte__Bint") And Contains(picoBlockMemoryC, "bbMemClear(") And Contains(picoBlockMemoryC, "bbMemCopy(") And Contains(picoBlockMemoryC, "bbMemMove("), "Pico lowers managed Array payloads and standard allocation, clearing, copying, moving, extension, and release calls through the embedded ABI")
...
Check(picoReferenceArray.Succeeded() And picoReferenceArrayDiagnostics.length = 0 And Contains(picoReferenceArrayC, "BMX_PICO_ARRAY_ELEMENT_STRING"), "Pico managed-container profile emits String Array element metadata")
...
Check(picoObjectArray.Succeeded() And picoObjectArrayDiagnostics.length = 0 And Contains(picoObjectArrayC, "BMX_PICO_ARRAY_ELEMENT_OBJECT") And Contains(picoObjectArrayC, "BMXPicoRootSlot bmx_pico_root_slots") And Contains(picoObjectArrayC, "BMX_PICO_ROOT_ARRAY"), "Pico Object Arrays emit precise element metadata and typed Array roots")
...
Check(picoObject.Succeeded() And picoObjectDiagnostics.length = 0 And Contains(picoObjectC, "BMXPicoObject object;") And Contains(picoObjectC, "static const BMXPicoTypeDescriptor bmx_pico_type_") And Contains(picoObjectC, "bmx_pico_object_allocate(&bmx_pico_type_") And Contains(picoObjectC, "bmx_pico_object_not_null(") And Contains(picoObjectC, "BMXPicoRootFrame bmx_pico_root_frame;") And Contains(picoObjectC, "bmx_pico_root_frame_enter(") And Contains(picoObjectC, "bmx_pico_root_frame_leave(") And Contains(picoObjectC, "bmx_fn1_Add("), "Pico standalone C emits described arena-backed concrete Objects with safepoint-driven root frames, cheap release receiver checks, constructors, fields and virtual method calls")
...
Check(picoDebugObject.Succeeded() And picoDebugObjectDiagnostics.length = 0 And Contains(picoDebugObjectC, "bmx_pico_object_assert(") And Contains(picoDebugObjectC, "bmx_pico_root_frame_enter("), "Pico debug builds retain full heap/descriptor receiver validation and conservative root frames: " + CompilerDiagnosticSummary(picoDebugObject) + "; emission: " + EmissionDiagnosticSummary(picoDebugObjectDiagnostics))
...
Check(picoFinalCall.Succeeded() And picoFinalCallDiagnostics.length = 0 And Contains(picoFinalCallC, "bmx_pico_object_not_null(") And Not Contains(picoFinalCallC, ")->type->methods["), "Pico calls on a statically Final receiver devirtualize to its exact implementation while retaining the release null check")
...
Check(picoInheritance.Succeeded() And picoInheritanceDiagnostics.length = 0 And Contains(picoInheritanceC, ".super = &bmx_pico_type_") And Contains(picoInheritanceC, "static const BMXPicoMethod") And Contains(picoInheritanceC, "->type->methods[") And Contains(picoInheritanceC, "bmx_fn0_Score(") And Contains(picoInheritanceC, "bmx_fn3_Score(") And Contains(picoInheritanceC, "bmx_pico_finalize_cls0_TBase(object);") And Contains(picoInheritanceC, "bmx_pico_finalize_cls1_TDerived"), "Pico local inheritance supports abstract bases and emits base-first layouts, compact method tables, virtual override dispatch, direct Super calls and derived-to-base finalizer wrappers")
...
Check(picoInterface.Succeeded() And picoInterfaceDiagnostics.length = 0 And Contains(picoInterfaceC, "static const BMXPicoInterfaceDescriptor") And Contains(picoInterfaceC, "static const BMXPicoInterfaceEntry") And Contains(picoInterfaceC, ".interfaces =") And Contains(picoInterfaceC, "bmx_pico_interface_methods(") And Contains(picoInterfaceC, "bmx_pico_interface_cast("), "Pico local Interfaces use erased Object references, compact per-Type tables, inherited overrides and checked casts")
...
Check(Contains(picoImportedInterfaceC, "{ ~qIReading~q, ~qpico_contracts_IReading~q }") And Contains(picoImportedInterfaceC, "{ ~qILabelled~q, ~qpico_contracts_ILabelled~q }") And Occurrences(picoImportedInterfaceC, "static const BMXPicoInterfaceEntry") >= 1 And Contains(picoImportedInterfaceC, "bmx_pico_interface_methods(") And Contains(picoImportedInterfaceC, "bmx_pico_interface_cast(") And Contains(picoImportedInterfaceC, "BMX_PICO_EXCEPTION_OBJECT"), "Pico materializes imported Interface descriptors from stable published ABI names for inheritance, implementation, dispatch, casts, and catches")
...
Check(Contains(picoImportedTypeC, "struct pico_types_TImportedReading_obj;") And Contains(picoImportedTypeC, ".abi_name = ~qpico_types_TImportedReading~q") And Contains(picoImportedTypeC, "_pico_types_TImportedReading_New__Bint_ObjectNew(40)") And Contains(picoImportedTypeC, "((struct pico_types_TImportedReading_obj *)&bmx_pico_null_object)") And Not Contains(picoImportedTypeC, "((struct pico_types_TImportedReading_obj *)&bbNullObject)") And Contains(picoImportedTypeC, "bmx_pico_type_methods(") And Contains(picoImportedTypeC, "bmx_pico_object_cast(") And Contains(picoImportedTypeC, "BMX_PICO_ROOT_OBJECT"), "Pico materializes stable imported Type identities, uses the embedded null sentinel, calls producer-owned construction helpers, and dispatches through the producer-owned compact method table")
...
Check(picoImportedField.Succeeded() And picoImportedFieldDiagnostics.length = 0 And Contains(picoImportedFieldC, "struct pico_types_TImportedReading_obj {") And Contains(picoImportedFieldC, "BMXPicoObject object;") And Contains(picoImportedFieldC, "int32_t _pico_types_timportedreading_value;") And Contains(picoImportedFieldC, "const BMXPicoString * _pico_types_timportedreading_label;") And Contains(picoImportedFieldC, "->_pico_types_timportedreading_value") And Contains(picoImportedFieldC, "->_pico_types_timportedreading_label") And Contains(picoImportedFieldC, "bmx_pico_object_not_null("), "Pico reconstructs the current imported Type layout for null-checked public scalar and managed field reads and writes")
...
Check(AppearsBefore(picoImportedBaseC, "_pico_types_timportedreading_value", "bmx_field_f0_extra") And Contains(picoImportedBaseC, ".super = &bmx_pico_imported_type_") And Contains(picoImportedBaseC, "_pico_types_TImportedReading_New__Bint((struct pico_types_TImportedReading_obj *)bmx_self_self, bmx_p0_initial);") And Contains(picoImportedBaseC, "_pico_types_TImportedReading_Read__Bint((struct pico_types_TImportedReading_obj *)bmx_self_self") And Contains(picoImportedBaseC, "interfaces_0[1] = {~n    (BMXPicoMethod)_pico_types_TImportedReading_Stable") And Contains(picoImportedBaseC, "bmx_pico_interface_methods") And Contains(picoImportedBaseC, "_pico_types_TImportedReading_PicoBaseFinalize(object);") And Contains(picoImportedBaseC, "bmx_pico_object_cast("), "Pico imported-base layouts preserve the base prefix and connect construction, inherited and overridden dispatch, inherited Interface implementations, Super calls, casts, and finalizer chaining")
...
Check(Contains(picoClosureC, "BMXPicoClosure *") And Contains(picoClosureC, "bmx_pico_closure_allocate(") And Contains(picoClosureC, "bmx_pico_closure_assert(") And Contains(picoClosureC, "BMX_PICO_ROOT_OBJECT"), "Pico emits precisely rooted managed Closure values and captured environments")
...
Check(Contains(picoExceptionC, "BMXPicoExceptionFrame") And Contains(picoExceptionC, "switch (setjmp(") And Contains(picoExceptionC, "bmx_pico_exception_enter(") And Contains(picoExceptionC, "bmx_pico_exception_catch(") And Contains(picoExceptionC, "bmx_pico_exception_throw(") And Contains(picoExceptionC, "bmx_pico_exception_leave("), "Pico exceptions lower through the embedded setjmp frame ABI")
Check(Contains(picoExceptionC, "bmx_pico_object_cast(") And Contains(picoExceptionC, "bmx_pico_interface_cast(") And Contains(picoExceptionC, "BMX_PICO_ROOT_EXCEPTION") And Not Contains(picoExceptionC, "bbEx"), "Pico typed catches use local descriptors and a precisely rooted tagged pending exception without desktop exception calls")
...
Check(picoStringException.Succeeded() And picoStringExceptionDiagnostics.length = 0 And Contains(picoStringExceptionC, "bmx_pico_exception_string(") And Contains(picoStringExceptionC, ".kind == BMX_PICO_EXCEPTION_STRING"), "Pico String Throw and Catch use the tagged exception carrier without changing the compact String layout")
...
Check(picoArrayException.Succeeded() And picoArrayExceptionDiagnostics.length = 0 And Contains(picoArrayExceptionC, "bmx_pico_exception_array(") And Contains(picoArrayExceptionC, ".kind == BMX_PICO_EXCEPTION_ARRAY"), "Pico Array Throw and Catch use the tagged exception carrier without changing the compact Array layout")
...
Check(Contains(picoUsingC, "ICloseable") And Contains(picoUsingC, "bmx_pico_interface_methods(") And Occurrences(picoUsingC, "switch (setjmp(") >= 3 And Occurrences(picoUsingC, "(void)bmx_pico_exception_catch();") = 2 And Contains(picoUsingC, "_failed) bmx_pico_exception_throw("), "Pico Using protects its body, closes resources in guarded frames, suppresses Close exceptions, and rethrows the original tagged exception")
...
Check(picoUsingReturn.Succeeded() And picoUsingReturnDiagnostics.length = 0 And Contains(picoUsingReturnC, "bmx_cleanup_return_0 = 41;") And AppearsBefore(picoUsingReturnC, "bmx_pico_exception_leave();", "return bmx_cleanup_return_0;") And AppearsBefore(picoUsingReturnC, "bmx_pico_interface_methods(", "return bmx_cleanup_return_0;"), "Pico Return across Using preserves its value, leaves the body handler, closes the resource, and then returns")
...
Check(picoObjectValues.Succeeded() And picoObjectValueDiagnostics.length = 0 And Contains(picoObjectValueC, ".compare = bmx_pico_compare_") And Contains(picoObjectValueC, ".hash_code = bmx_pico_hash_code_") And Contains(picoObjectValueC, ".equals = bmx_pico_equals_") And Occurrences(picoObjectValueC, "bmx_pico_object_compare(") >= 2 And Occurrences(picoObjectValueC, "bmx_pico_object_hash_code(") >= 3 And Occurrences(picoObjectValueC, "bmx_pico_object_equals(") >= 3, "Pico Object value operations use compact descriptor hooks with runtime defaults through concrete and Object-typed receivers")
...
Check(picoReferenceField.Succeeded() And picoReferenceFieldDiagnostics.length = 0 And Contains(picoReferenceFieldC, "static const uint32_t bmx_pico_type_") And Contains(picoReferenceFieldC, "(uint32_t)offsetof(struct ") And Contains(picoReferenceFieldC, ".reference_count = 1"), "Pico embedded Object fields emit precise 32-bit descriptor offsets")
...
Check(picoBaseObject.Succeeded() And picoBaseObjectDiagnostics.length = 0 And Contains(picoBaseObjectC, "BMXPicoObject *") And Contains(picoBaseObjectC, "&bmx_pico_null_object") And Not Contains(picoBaseObjectC, "bbNullObject"), "Pico base Object values use the embedded object ABI and null sentinel")
...
Check(picoManagedStringField.Succeeded() And picoManagedStringFieldDiagnostics.length = 0 And Contains(picoManagedStringFieldC, "const BMXPicoString *") And Contains(picoManagedStringFieldC, "_strings[]") And Contains(picoManagedStringFieldC, ".string_count = 1"), "Pico embedded Objects publish precise managed String field offsets")
...
Check(picoManagedPublicStructConsumer.Succeeded() And picoManagedPublicStructConsumerDiagnostics.length = 0 And Contains(picoManagedPublicStructConsumerC, "static const BMXPicoValueDescriptor bmx_pico_imported_value_") And Contains(picoManagedPublicStructConsumerC, "BMX_PICO_VALUE_STRING") And Contains(picoManagedPublicStructConsumerC, "BMX_PICO_VALUE_ARRAY") And Contains(picoManagedPublicStructConsumerC, "BMX_PICO_VALUE_OBJECT") And Contains(picoManagedPublicStructConsumerC, "BMX_PICO_ARRAY_ELEMENT_VALUE, bbStructElementInit_acme_managedvalues_SManagedValue, &bmx_pico_imported_value_") And Contains(picoManagedPublicStructConsumerC, "BMX_PICO_VALUE_STRUCT, &bmx_pico_imported_value_") And Contains(picoManagedPublicStructConsumerC, "(uint32_t)sizeof(struct acme_managedvalues_SManagedValue), 2, BMX_PICO_VALUE_STRUCT") And Contains(picoManagedPublicStructConsumerC, "BMX_PICO_ROOT_STRUCT, &bmx_pico_imported_value_") And Contains(picoManagedPublicStructConsumerC, "_roots[0]") And Contains(picoManagedPublicStructConsumerC, "_roots[1]"), "Pico imported managed Struct fields, dynamic Arrays and StaticArrays share recursive 32-bit descriptors across roots, nested Structs and containing Objects")
