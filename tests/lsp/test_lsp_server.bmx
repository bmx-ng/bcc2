...
Check(Not configuration.noAutoSuperStrict, "LSP defaults undeclared source mode to SuperStrict")
Local automaticLspMode:TLanguageAnalysis = TBlitzMaxLanguage.AnalyzeText("Function Probe()~nEnd Function", "automatic-lsp-mode.bmx", configuration.AnalysisOptions())
Check(automaticLspMode.model.globalScope.LookupLocal("Probe")[0].declaredType = automaticLspMode.model.BuiltinType("Void"), "LSP direct analysis applies automatic SuperStrict")
configuration.sdkPath = TestSdkPath()
configuration.ApplyJson(ObjectFrom("{~qbuildMode~q:~qdebug~q,~qrootSourcePath~q:~q/workspace/main.bmx~q,~quseDependencySnapshots~q:false,~qwarnImplicitDefaultReturns~q:true,~qnoAutoSuperStrict~q:true,~qconditionalSymbols~q:[~qcustom~q]}"))
...
Check(configuration.noAutoSuperStrict And configuration.Copy().noAutoSuperStrict And configuration.SnapshotOptions().noAutoSuperStrict And configuration.AnalysisOptions().noAutoSuperStrict, "LSP -nas setting reaches both snapshot and direct analysis")
Local disabledAutomaticLspMode:TLanguageAnalysis = TBlitzMaxLanguage.AnalyzeText("Function Probe()~nEnd Function", "no-automatic-lsp-mode.bmx", configuration.AnalysisOptions())
Check(disabledAutomaticLspMode.model.globalScope.LookupLocal("Probe")[0].declaredType = disabledAutomaticLspMode.model.BuiltinType("Int"), "LSP direct analysis honours the -nas workspace setting")
...
Local selfAssignmentRange:TJSONObject = TJSONObject(selfAssignmentDiagnostic.Get("range"))
Local requestedSelfAssignmentDiagnostics:TJSONArray = JsonArray()
requestedSelfAssignmentDiagnostics.Append(selfAssignmentDiagnostic)
responses = selfAssignmentServer.HandlePayload(CodeActionPayload(76, selfAssignmentUri, selfAssignmentRange, requestedSelfAssignmentDiagnostics, "quickfix"))
Local selfAssignmentActions:TJSONArray = TJSONArray(ObjectFrom(responses[0]).Get("result"))
Check(selfAssignmentActions.Size() = 1, "a hidden-field self-assignment offers one quick fix")
Local qualifySelfAction:TJSONObject = TJSONObject(selfAssignmentActions.Get(0))
Local qualifySelfEdits:TJSONArray = WorkspaceEditEdits(TJSONObject(qualifySelfAction.Get("edit")), selfAssignmentUri)
Local qualifySelfEdit:TJSONObject = TJSONObject(qualifySelfEdits.Get(0))
Local qualifySelfEditRange:TJSONObject = TJSONObject(qualifySelfEdit.Get("range"))
Check(qualifySelfAction.GetString("title") = "Qualify field with Self" And qualifySelfAction.GetBool("isPreferred") And qualifySelfEdits.Size() = 1 And qualifySelfEdit.GetString("newText") = "Self.", "the preferred self-assignment fix inserts only the receiver qualifier")
Check(TBlitzMaxLspCodeActions.SameRange(qualifySelfEditRange, PointRange(4, 0)), "the self-assignment fix inserts at the start of the written left-hand name")

Local ordinarySelfAssignmentUri:String = "file:///tmp/ordinary-self-assignment-warning.bmx"
responses = selfAssignmentServer.HandlePayload(DidOpenPayload(ordinarySelfAssignmentUri, "SuperStrict~nLocal amount:Int~namount=amount"))
Local ordinarySelfAssignmentDiagnostics:TJSONArray = TJSONArray(TJSONObject(ObjectFrom(responses[0]).Get("params")).Get("diagnostics"))
Local ordinarySelfAssignmentDiagnostic:TJSONObject = FindProtocolDiagnostic(ordinarySelfAssignmentDiagnostics, "BMX3411")
Local requestedOrdinarySelfAssignmentDiagnostics:TJSONArray = JsonArray()
requestedOrdinarySelfAssignmentDiagnostics.Append(ordinarySelfAssignmentDiagnostic)
responses = selfAssignmentServer.HandlePayload(CodeActionPayload(77, ordinarySelfAssignmentUri, TJSONObject(ordinarySelfAssignmentDiagnostic.Get("range")), requestedOrdinarySelfAssignmentDiagnostics, "quickfix"))
Check(TJSONArray(ObjectFrom(responses[0]).Get("result")).Size() = 0, "an ordinary Local self-assignment warning does not offer an invalid field-qualification fix")
...
Check(implementSnippet.Contains("Method Required:String(value:String Var) Override") And implementSnippet.Contains("Method Apply:Int(value:Int, text:String, values:Int[], objectValue:Object, interfaceValue:IMarker, structValue:SPoint, callback:Closure<Int(input:Int)>) Override"), "generated stubs substitute inherited generic scalar, String, Array, Object, Interface, Struct, Closure and Var parameter types and mark instance Methods as overrides")
Check(implementSnippet.Contains("Method Nested:TBox<Int>(value:TBox<Int>) Override") And implementSnippet.Contains("Throw ~q${1:Not implemented}~q") And implementSnippet.EndsWith("$0"), "generated stubs preserve nested constructed types and expose editable method bodies")
...
responses = completionServer.HandlePayload("{~qjsonrpc~q:~q2.0~q,~qid~q:461,~qmethod~q:~qtextDocument/completion~q,~qparams~q:{~qtextDocument~q:{~quri~q:~qfile:///tmp/completion.bmx~q},~qposition~q:{~qline~q:24,~qcharacter~q:9},~qcontext~q:{~qtriggerKind~q:2,~qtriggerCharacter~q:~q.~q}}}")
completionResponse = ObjectFrom(responses[0])
completionItems = TJSONArray(completionResponse.Get("result"))
Check(CompletionItemCount(completionItems, "Calc") = 2 And FindCompletionItem(completionItems, "value") <> Null, "member completion recovers a typed receiver inside a parenthesized call argument")
responses = completionServer.HandlePayload("{~qjsonrpc~q:~q2.0~q,~qid~q:462,~qmethod~q:~qtextDocument/completion~q,~qparams~q:{~qtextDocument~q:{~quri~q:~qfile:///tmp/completion.bmx~q},~qposition~q:{~qline~q:30,~qcharacter~q:16},~qcontext~q:{~qtriggerKind~q:2,~qtriggerCharacter~q:~q.~q}}}")
completionResponse = ObjectFrom(responses[0])
completionItems = TJSONArray(completionResponse.Get("result"))
Check(FindCompletionItem(completionItems, "one") <> Null And FindCompletionItem(completionItems, "two") <> Null, "member completion recovers a forward-typed Global receiver inside a call argument")
...
Check(unitWorkspace.LatestAnalysis("file://" + unitLeafPath).model.globalScope.LookupLocal("IncludedProcedure")[0].declaredType = unitWorkspace.LatestAnalysis("file://" + unitLeafPath).model.BuiltinType("Void"), "unknown include opened alone uses automatic SuperStrict mode")
...
responses = unitServer.HandlePayload("{~qjsonrpc~q:~q2.0~q,~qid~q:92,~qmethod~q:~qtextDocument/references~q,~qparams~q:{~qtextDocument~q:{~quri~q:~qfile://" + unitLeafPath + "~q},~qposition~q:{~qline~q:1,~qcharacter~q:8},~qcontext~q:{~qincludeDeclaration~q:true}}}")
Local unitRootReferences:TJSONArray = TJSONArray(ObjectFrom(responses[0]).Get("result"))
Check(unitRootReferences.Size() = 2 And HasLocation(unitRootReferences, "file://" + unitRootPath, 1, 9) And HasLocation(unitRootReferences, "file://" + unitLeafPath, 1, 7), "references span a root declaration and its use in a recursive Include")
responses = unitServer.HandlePayload("{~qjsonrpc~q:~q2.0~q,~qid~q:93,~qmethod~q:~qtextDocument/references~q,~qparams~q:{~qtextDocument~q:{~quri~q:~qfile://" + unitLeafPath + "~q},~qposition~q:{~qline~q:1,~qcharacter~q:22},~qcontext~q:{~qincludeDeclaration~q:true}}}")
Local unitMiddleReferences:TJSONArray = TJSONArray(ObjectFrom(responses[0]).Get("result"))
Check(unitMiddleReferences.Size() = 2 And HasLocation(unitMiddleReferences, "file://" + unitMiddlePath, 0, 9) And HasLocation(unitMiddleReferences, "file://" + unitLeafPath, 1, 21), "references include declarations from unopened intermediate Include files")
